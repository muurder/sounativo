...
const compressImage = async (file: File): Promise<File> => {
  try {
    ...
    logger.info(
      `[ImageCompression] ${file.name}: ${originalSizeMB}MB → ${compressedSizeMB}MB (${reduction}% reduction)`
    );
...
const CreateTripWizard: React.FC<CreateTripWizardProps> = ({
  ...
  const [frequentBoardingPoints, setFrequentBoardingPoints] = useState<
  ...
  const [tripData, setTripData] = useState<Partial<Trip>>(() => {
    ...
    if (initialTripData?.id) {
      const initialWithDefaults = {
        ...initialTripData,
        categories:
          initialTripData.categories ||
          (initialTripData.category ? [initialTripData.category] : ['PRAIA']),
        category:
          initialTripData.category ||
          (initialTripData.categories && initialTripData.categories[0]) ||
          'PRAIA',
      };
      return {
  ...
  useEffect(() => {
    const fetchFrequentBoardingPoints = async () => {
      ...
      try {
        if (typeof guardSupabase !== 'function') {
          logger.warn('guardSupabase is not available yet');
          return;
        }
  ...
  useEffect(() => {
    if (isEditing && initialTripData) {
      const formattedStartDate = initialTripData.startDate
        ? new Date(initialTripData.startDate).toISOString().split('T')[0]
        : '';
      const formattedEndDate = initialTripData.endDate
        ? new Date(initialTripData.endDate).toISOString().split('T')[0]
        : '';
  ...
  const validateStep = useCallback(() => {
    ...
    if (currentStep === 0) {
      ...
      if (totalImages > MAX_IMAGES)
        newErrors.images = `Máximo de ${MAX_IMAGES} imagens permitidas. Você tem ${totalImages}.`;
      if (!tripData.description || tripData.description.length < 20)
        newErrors.description = 'Escreva uma descrição (min 20 caracteres).';
    } else if (currentStep === 2) {
      // Passenger Config (optional, no validation needed)
  ...
  const handlePublish = async () => {
    ...
    if (
      ...
      tripData.price === undefined ||
      tripData.price === null ||
      isNaN(tripData.price) ||
      tripData.price <= 0 ||
      !tripData.startDate ||
      !tripData.endDate
    ) {
    ...
    try {
      ...
      const publishPromise = (async () => {
              ...
              const timeoutPromise = new Promise<never>((_, reject) =>
                setTimeout(
                  () =>
                    reject(
                      new Error(
                        `Upload timeout: ${file.name} demorou mais de 90 segundos. Tente uma imagem menor ou verifique sua conexão.`
                      )
                    ),
                  90000
                )
        ...
        if (finalImages.length > MAX_IMAGES) {
          throw new Error(
            `Máximo de ${MAX_IMAGES} imagens permitidas. Você tem ${finalImages.length}. Remova algumas imagens antes de publicar.`
          );
        }
        ...
        const savePromise = isEditing ? updateTrip(finalTrip) : createTrip(finalTrip);
        ...
        if (pendingFailedUploads.length > 0) {
          showToast(
            `Pacote ${isEditing ? 'atualizado' : 'criado'} com sucesso! Algumas fotos falharam: ${pendingFailedUploads.join(', ')}`,
            'warning'
          );
        } else {
  ...
  const addBoardingPoint = () => {
    setTripData((prev) => ({
      ...
      boardingPoints: [
        ...(prev.boardingPoints || []),
        { id: crypto.randomUUID(), time: '08:00', location: '' },
      ],
    }));
  };
    ...
    return (
      <div className="space-y-6 animate-[fadeIn_0.3s]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            <div className="md:col-span-2">
              ...
              <input
                ...
                onChange={(e) =>
                  setTripData({ ...tripData, title: e.target.value, slug: slugify(e.target.value) })
                }
        ...
        <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setTripData({ ...tripData, durationDays: parseInt(e.target.value) || 1 })
                    }
                ...
                <button
                  ...
                  onClick={(e) => {
                    ...
                    try {
                      ...
                      logger.log(
                        `[CreateTripWizard] Recalculating duration: ${normalizedStartDate} to ${normalizedEndDate} = ${finalDays} days`
                      );
          ...
          {errors.dates && (
            <p className="text-red-500 text-sm font-bold bg-red-50 p-3 rounded-lg mt-4">
              {errors.dates}
            </p>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-3">
              Categorias{' '}
              <span className="text-xs font-normal text-gray-500">(Selecione uma ou mais)</span>
            </label>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2.5 p-4 border border-gray-200 rounded-lg bg-gray-50">
              {ALL_TRIP_CATEGORIES.map((cat) => {
                ...
                const currentCategories =
                  tripData.categories || (tripData.category ? [tripData.category] : []);
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-xs font-semibold text-center leading-tight px-1 ${isSelected ? 'text-white' : 'text-gray-700'}`}
                    >
                      {cat.replace(/_/g, ' ')}
                    </span>
                  </label>
            ...
            {tripData.categories && tripData.categories.length > 0 && (
              <p className="text-xs text-gray-500 mt-3 flex items-center gap-1.5">
                ...
                <span className="font-medium">
                  {tripData.categories.length} categoria{tripData.categories.length > 1 ? 's' : ''}{' '}
                  selecionada{tripData.categories.length > 1 ? 's' : ''}
                </span>
              </p>
            )}
          </div>
        ...
        <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
          ...
          <p className="text-sm text-gray-600 mb-4">
            Defina os pontos de saída e horários de embarque
          </p>
              ...
              <div className="flex flex-wrap gap-2">
                {frequentBoardingPoints.map((bp, idx) => {
                  ...
                  const isAlreadyAdded = tripData.boardingPoints?.some(
                    (existing) =>
                      existing.location.toLowerCase().trim() === bp.location.toLowerCase().trim()
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[10px] text-gray-400 group-hover:text-primary-500">
                        ({bp.count}x)
                      </span>
                    </button>
                  );
          ...
          <div className="space-y-3">
            {tripData.boardingPoints?.map((bp, idx) => (
              <div
                key={bp.id}
                className="flex gap-3 items-center bg-gray-50 p-3 rounded-lg border border-gray-200 hover:border-primary-300 transition-colors"
              >
  ...
  const renderStep2 = () => {
    const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files && e.target.files.length > 0) {
        ...
        if (availableSlots <= 0) {
          showToast(
            `Limite de ${MAX_IMAGES} imagens atingido. Remova algumas imagens antes de adicionar novas.`,
            'warning'
          );
        ...
        if (newFiles.length > availableSlots) {
          showToast(
            `Você pode adicionar apenas mais ${availableSlots} imagem(ns). ${newFiles.length - availableSlots} imagem(ns) foram ignoradas.`,
            'warning'
          );
      ...
      <div className="space-y-6 animate-[fadeIn_0.3s]">
        ...
        <div className="bg-gray-50 p-5 rounded-xl border border-gray-200">
          <div className="flex items-center justify-between mb-3">
            <label className="block text-sm font-bold text-gray-700">
              Fotos da Viagem <span className="text-red-500">*</span>
            </label>
            ...
            {tripData.images?.map((url, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Drag Handle - Premium */}
                  <div
                    className={`absolute left-2 top-2 w-8 h-8 bg-white/95 backdrop-blur-md rounded-lg flex items-center justify-center shadow-xl transition-all z-20 ${
                  ...
                  <div
                    className={`absolute bottom-2 left-2 bg-gradient-to-r from-primary-600 to-primary-700 text-white text-xs font-bold px-2.5 py-1.5 rounded-lg shadow-lg backdrop-blur-sm transition-all z-20 ${
            ...
            {filesToUpload.map((file, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Position Indicator - Premium */}
                  <div
                    className={`absolute bottom-2 left-2 bg-gradient-to-r from-amber-600 to-amber-700 text-white text-xs font-bold px-2.5 py-1.5 rounded-lg shadow-lg backdrop-blur-sm transition-all z-20 ${
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {[...DEFAULT_SUGGESTED_TAGS, ...(currentAgency?.customSettings?.tags || [])].map(
              (tag) => (
                <button
                  key={tag}
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {tripData.tags?.map((tag) => (
              <span
                key={String(tag)}
                className="bg-primary-50 text-primary-700 text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1"
              >
                {String(tag)}
                <button
                  onClick={() =>
          ...
          <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border border-green-200 shadow-sm">
            ...
            <div className="space-y-2.5 max-h-64 overflow-y-auto custom-scrollbar pr-1">
              {tripData.included?.map((item, idx) => (
                <div
                  key={idx}
                  className="group flex items-center justify-between bg-white rounded-lg p-3.5 border border-green-200 shadow-sm hover:shadow-md hover:border-green-300 transition-all"
                >
          ...
          <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl p-6 border border-amber-200 shadow-sm">
            ...
            <div className="space-y-2.5 max-h-64 overflow-y-auto custom-scrollbar pr-1">
              {tripData.notIncluded?.map((item, idx) => (
                <div
                  key={idx}
                  className="group flex items-center justify-between bg-white rounded-lg p-3.5 border border-amber-200 shadow-sm hover:shadow-md hover:border-amber-300 transition-all"
                >
                  <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              {(!tripData.notIncluded || tripData.notIncluded.length === 0) && (
                <div className="text-center py-8 bg-white/50 rounded-lg border-2 border-dashed border-amber-200">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Adicione itens não incluídos (opcional)
                  </p>
                </div>
              )}
            </div>
                ...
                onClick={() => {
                  ...
                  setTimeout(() => {
                    const lastInput = document.querySelector(
                      `input[placeholder*="Dia ${currentDays + 1}"]`
                    ) as HTMLInputElement;
                    if (lastInput) {
                      lastInput.focus();
                      lastInput.select();
        ...
        <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg border border-gray-200">
          ...
          <div>
            ...
            <span className="text-xs text-gray-500">
              Se desmarcado, ficará salvo como rascunho.
            </span>
          </div>
        </div>
      </div>
                ...
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Configure permissões para menores de idade
                    </p>
                  </div>
                </div>
                            ...
                            setTripData({
                              ...
                              passengerConfig: {
                                ...
                                childPriceMultiplier:
                                  tripData.passengerConfig?.childPriceMultiplier ?? 1.0,
                              },
                            })
                          }
                    ...
                    <div className="p-5 bg-gray-50 rounded-xl border border-gray-200">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <p className="text-sm text-gray-600">
                          anos ou menos são considerados crianças
                        </p>
                      </div>
                    </div>
                  )}
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-base font-semibold text-gray-900">
                            Permitir criança no colo
                          </p>
                          <p className="text-sm text-gray-600">
                            Crianças pequenas podem viajar sem assento próprio (até 2 anos)
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 hover:shadow-md transition-shadow">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Configure permissões para terceira idade
                    </p>
                  </div>
                </div>
                ...
                <div className="p-5 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        Aceita passageiros da terceira idade (60+ anos)
                      </p>
                    </div>
                  </div>
                </div>
                    ...
                    {tripData.passengerConfig?.allowChildren !== false && (
                      <div className="flex items-center gap-2 text-sm text-gray-700 bg-purple-50 px-4 py-3 rounded-xl border border-purple-200">
                        ...
                        <span className="font-semibold">
                          ...
                          tripData.passengerConfig?.childPriceFixed !== undefined
            ...
            <div className="flex items-start gap-5 mb-8">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600 leading-relaxed">
                  Configure o desconto aplicado ao preço da viagem para crianças. Escolha entre
                  desconto percentual ou valor fixo em reais.
                </p>
              </div>
            </div>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const defaultDiscount = Math.round(
                      currentPrice * (currentDiscountPercent / 100)
                    );
                  ...
                  <div className="space-y-5">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mb-4">
                        Digite a porcentagem de desconto sobre o preço da viagem
                      </p>
                    </div>
                    <div className="relative bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl p-6 border-2 border-emerald-200">
                      <input
                        ...
                        step="1"
                        value={Math.round(
                          (1 - (tripData.passengerConfig?.childPriceMultiplier ?? 1.0)) * 100
                        )}
                        onChange={(e) => {
                    ...
                    <div className="flex items-start gap-3 text-sm text-gray-700 bg-emerald-50 p-4 rounded-xl border border-emerald-200">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-600">
                          ...
                          % de desconto sobre o preço da viagem
                        </p>
                      </div>
                    </div>
                  </div>
                            ...
                            <div className="flex items-center gap-2">
                              <div className="w-6 h-6 rounded-lg bg-emerald-500 flex items-center justify-center">
                                <span className="text-white text-xs font-bold">
                                  -
                                  {Math.round(
                                    (1 - (tripData.passengerConfig?.childPriceMultiplier ?? 1.0)) *
                                      100
                                  )}
                                  %
                                </span>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div className="space-y-5">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mb-4">
                        Digite o valor do desconto em reais sobre o preço da viagem
                      </p>
                    </div>
                    <div className="relative bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl p-6 border-2 border-emerald-200">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const discount = Math.max(
                            0,
                            Math.min(price, parseFloat(e.target.value) || 0)
                          );
                    ...
                    <div className="flex items-start gap-3 text-sm text-gray-700 bg-emerald-50 p-4 rounded-xl border border-emerald-200">
                      ...
                      <div>
                        <p className="font-semibold mb-1">
                          ...
                          {tripData.passengerConfig?.childPriceFixed
                            ? Math.round(
                                (tripData.price || 500) - tripData.passengerConfig.childPriceFixed
                              ).toLocaleString('pt-BR')
                            : '0'}
                        </p>
                        <p className="text-xs text-gray-600">
        ...
        <div className="bg-white rounded-2xl max-w-md w-full p-8 shadow-2xl relative">
          ...
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">
              Não foi possível identificar sua agência. Por favor, recarregue a página.
            </p>
            <button
              onClick={onClose}
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-[fadeIn_0.2s]">
        <div
          className="bg-white rounded-2xl max-w-7xl w-full p-8 shadow-2xl relative max-h-[95vh] overflow-y-auto custom-scrollbar"
