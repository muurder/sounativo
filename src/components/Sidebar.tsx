...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  // Sync global navbar margin to prevent overlap
  useEffect(() => {
    const mainNav = document.getElementById('global-top-nav');
    if (mainNav) {
      if (isCollapsed) {
        mainNav.style.paddingLeft = '5rem'; // w-20 
      } else {
        mainNav.style.paddingLeft = '16rem'; // w-64 
      }
      mainNav.style.transition = 'padding-left 300ms ease-in-out';
    }
  ...
  return (
    ...
    >
      ...
      <div className="h-16 flex items-center justify-center border-b border-gray-100 relative">
        <div
          className={`flex items-center gap-2 transition-all duration-300 ${isCollapsed ? 'scale-75' : ''}`}
        >
        ...
        <button
          ...
          className="absolute -right-3 top-1/2 -translate-y-1/2 bg-white border border-gray-200 rounded-full p-1 shadow-sm hover:bg-gray-50 text-gray-500 hover:text-primary-600 transition-colors hidden md:block"
        >
      ...
      <div className="p-4 border-t border-gray-100 bg-gray-50/50">
        <div
          className={`flex items-center ${isCollapsed ? 'justify-center flex-col gap-4' : 'gap-3'}`}
        >
