...
const InteractiveTripCardComponent: React.FC<InteractiveTripCardProps> = ({ trip }) => {
  ...
  const images =
    tripWithImages.images && tripWithImages.images.length > 0 ? tripWithImages.images : [];
  ...
  const { agency, whatsappLink } = useMemo(() => {
    ...
    return {
      agency: foundAgency,
      whatsappLink: contactNumber
        ? `https://wa.me/55${contactNumber.replace(/\D/g, '')}?text=${encodeURIComponent(message)}`
        : null,
    };
    ...
    <div className="group bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col h-full relative">
        ...
        <div className="px-4 py-2 flex gap-1.5 overflow-x-auto scrollbar-hide border-b border-gray-50 h-[50px] items-center">
          {images.slice(0, 5).map((img, idx) => (
            <button
              ...
              className={`relative w-12 h-8 rounded-md overflow-hidden flex-shrink-0 transition-all ${
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <Link to={linkTarget} className="block mb-3">
          <h3
            className="text-lg font-bold text-gray-900 leading-tight line-clamp-2 group-hover:text-primary-600 transition-colors"
            title={tripWithImages.title}
          >
        ...
        <div className="grid grid-cols-5 gap-2 pt-4 border-t border-gray-100">
          ...
          {whatsappLink ? (
            ...
            <button
              disabled
              className="col-span-2 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-gray-100 text-gray-400 font-bold text-xs cursor-not-allowed"
            >
              <MessageCircle size={16} />
              Whats
            </button>
