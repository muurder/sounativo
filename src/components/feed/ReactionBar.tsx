...
const ReactionBar: React.FC<ReactionBarProps> = ({
  ...
  const [selectedEmoji, setSelectedEmoji] = useState<string | null>(() =>
    getStoredReaction(notificationId)
  );
    ...
    <div className={`flex items-center gap-1.5 ${className}`}>
      ...
      <div className="relative">
        <button
          ...
          disabled={isProcessing}
          className={`w-8 h-8 flex items-center justify-center rounded-lg text-stone-400 hover:bg-stone-100 hover:text-stone-600 transition-all ${
            isProcessing ? 'opacity-50 cursor-wait' : ''
          }`}
          title="Mais emojis"
        >
      ...
      {selectedEmoji && <span className="text-xs text-primary-600 font-medium ml-1">Reagido</span>}
    </div>
  );
};
