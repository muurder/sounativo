...
const MessageInbox: React.FC<MessageInboxProps> = ({
  ...
  return (
    <div
      className={`bg-white rounded-2xl border border-stone-200 shadow-sm overflow-visible ${className}`}
    >
      {/* Header with Tabs */}
      ...
      {currentList.length === 0 ? (
        ...
        <div className="p-8 text-center">
          {filterType === 'unread' ? (
            ...
            <div className="flex flex-col items-center">
              ...
              <p className="text-base font-semibold text-stone-700 mb-1">Tudo limpo por aqui!</p>
              <p className="text-sm text-stone-500">Nenhuma nova mensagem do sistema.</p>
              {totalRead > 0 && (
                <button
                  onClick={() => setFilterType('read')}
            ...
            <div className="flex flex-col items-center">
              ...
              <p className="text-sm text-stone-500">Você ainda não possui mensagens arquivadas.</p>
              {totalUnread > 0 && (
                <button
                  onClick={() => setFilterType('unread')}
                  className="mt-4 text-xs text-primary-600 hover:text-primary-700 font-medium flex items-center gap-1"
        ...
        <div className="p-4">
          ...
          >
            ...
            <p className="text-stone-700 text-sm leading-relaxed">{currentNotification.message}</p>
          </div>
