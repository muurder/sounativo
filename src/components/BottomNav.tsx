...
import { Home, Compass, Building, User, MapPin, Settings } from 'lucide-react';
...
const BottomNav: React.FC = () => {
  ...
  const NavItem = ({
    ...
    >
      <div className="relative">
        ...
        {active && (
          <div className="absolute -top-1 right-0 w-1.5 h-1.5 bg-primary-600 rounded-full md:hidden"></div>
        )}
      </div>
      <span>{label}</span>
    </Link>
  ...
  const isAccountActive = () => {
    if (!user && (pathname.includes('login') || window.location.hash.includes('login')))
      return true;
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] pb-[env(safe-area-inset-bottom)]">
      <div className="grid grid-cols-5 h-[64px]">
        ...
        />
        <NavItem to="/guides" icon={MapPin} label="Guias" active={pathname.startsWith('/guides')} />
        <NavItem to="/settings" icon={Settings} label="Ajustes" active={pathname === '/settings'} />
      </div>
    </div>
  );
};
