...
const LoginView: React.FC<any> = ({ setView, onClose, agencyContext }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // FIX: Ensure loading state is reset in both success and error paths
      setIsLoading(false);
    }
  };
    ...
    <div className="p-8 md:p-10">
      {agencyContext && (
        <div className="text-center mb-4">
          <img
            src={agencyContext.logo}
            className="w-14 h-14 rounded-full mx-auto mb-2 border-2 border-gray-100"
            alt={`${agencyContext.name} logo`}
          />
          <p className="text-xs text-gray-500">
            Acessando <span className="font-bold text-gray-700">{agencyContext.name}</span>
          </p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-1 focus:ring-primary-500 outline-none"
              required
            />
          </div>
        </div>
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-2.5 border border-gray-300 rounded-lg focus:ring-1 focus:ring-primary-500 outline-none"
              required
            />
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full bg-primary-600 text-white py-3 rounded-lg font-bold hover:bg-primary-700 disabled:opacity-50 transition-colors"
        >
      ...
      if (result.success) {
        ...
        setTimeout(() => {
          if (result.userId && result.role) {
            // Only navigate if user is signed in AND role is determined
    ...
    <div className="p-8 md:p-10 max-h-[90vh] overflow-y-auto scrollbar-thin">
      ...
      <div className="flex bg-gray-100 p-1 rounded-lg mb-6">
        <button
          ...
          className={`flex-1 py-2 text-sm font-medium rounded-md flex items-center justify-center gap-2 ${activeTab === 'CLIENT' ? 'bg-white shadow text-primary-600' : 'text-gray-500'}`}
        >
          <User size={16} /> Quero Viajar
        </button>
        <button
          onClick={() => setActiveTab('AGENCY')}
          className={`flex-1 py-2 text-sm font-medium rounded-md flex items-center justify-center gap-2 ${activeTab === 'AGENCY' ? 'bg-white shadow text-primary-600' : 'text-gray-500'}`}
        >
          <Building size={16} /> Sou Parceiro
        </button>
      </div>
      ...
      <GoogleButton
        label={
          activeTab === 'CLIENT' ? 'Cadastrar Viajante com Google' : 'Cadastrar Agência com Google'
        }
      ...
      {/* Terms Modal */}
      {showTermsModal && (
        <TermsPrivacyModal type="terms" onClose={() => setShowTermsModal(false)} />
      )}

      {/* Privacy Modal */}
      {showPrivacyModal && (
        <TermsPrivacyModal type="privacy" onClose={() => setShowPrivacyModal(false)} />
      )}
    </div>
  );
};
  ...
  >
    ...
    >
      <g transform="matrix(1, 0, 0, 1, 27.009001, -39.238998)">
        ...
        <path
          fill="#FBBC05"
          d="M -21.484 53.529 C -21.734 52.809 -21.864 52.039 -21.864 51.239 C -21.864 50.439 -21.734 49.669 -21.484 48.949 L -21.484 45.859 C -26.284 47.479 -26.754 49.299 -26.754 51.239 C -26.754 53.179 -26.284 54.999 -25.464 56.619 L -21.484 53.529 Z"
        />
        <path
          fill="#EA4335"
...
const TermsContent: React.FC = () => (
  <div className="prose prose-blue max-w-none text-gray-700">
    ...
    <p className="text-gray-700 leading-relaxed mb-4">
      Ao acessar e usar a plataforma SouNativo, você aceita e concorda em estar vinculado aos termos
      e disposições deste acordo. Se você não concorda com algum dos termos aqui estabelecidos, não
      deve utilizar nossos serviços.
    </p>

    <h3 className="text-xl font-bold text-gray-900 mt-6 mb-3">2. Descrição do Serviço</h3>
    <p className="text-gray-700 leading-relaxed mb-4">
      O SouNativo é uma plataforma intermediária que conecta agências de viagens, guias turísticos e
      prestadores de serviços turísticos a consumidores finais. Não somos responsáveis pela
      prestação direta dos serviços de viagem, mas facilitamos a conexão entre as partes.
    </p>

    <h3 className="text-xl font-bold text-gray-900 mt-6 mb-3">3. Cadastro e Conta de Usuário</h3>
    <p className="text-gray-700 leading-relaxed mb-4">
      Para utilizar nossos serviços, você precisa criar uma conta fornecendo informações precisas e
      atualizadas. Você é responsável por manter a confidencialidade de suas credenciais e por todas
      as atividades que ocorram em sua conta.
    </p>

    <h3 className="text-xl font-bold text-gray-900 mt-6 mb-3">4. Reservas e Pagamentos</h3>
    <p className="text-gray-700 leading-relaxed mb-4">
      Ao realizar uma reserva através do SouNativo, você concorda em pagar o valor total indicado.
      Os pagamentos são processados de forma segura através de gateways certificados. A confirmação
      da reserva está sujeita à disponibilidade e aprovação do prestador.
    </p>

    <h3 className="text-xl font-bold text-gray-900 mt-6 mb-3">5. Cancelamentos e Reembolsos</h3>
    <p className="text-gray-700 leading-relaxed mb-4">
      As políticas de cancelamento e reembolso variam conforme a agência e o pacote contratado. Leia
      atentamente as condições específicas na página de cada viagem antes de confirmar sua reserva.
      O SouNativo atua como intermediária e não se responsabiliza por políticas de cancelamento
      definidas pelos prestadores.
    </p>

    <h3 className="text-xl font-bold text-gray-900 mt-6 mb-3">6. Responsabilidades</h3>
    <p className="text-gray-700 leading-relaxed mb-4">
      O SouNativo não se responsabiliza por atrasos, cancelamentos, mudanças de itinerário ou
      qualquer problema relacionado aos serviços prestados pelas agências parceiras. Nossa
      responsabilidade limita-se à intermediação da transação.
    </p>

    <h3 className="text-xl font-bold text-gray-900 mt-6 mb-3">7. Propriedade Intelectual</h3>
    <p className="text-gray-700 leading-relaxed mb-4">
      Todo o conteúdo da plataforma, incluindo textos, imagens, logotipos e design, é propriedade do
      SouNativo ou de seus licenciadores e está protegido por leis de propriedade intelectual.
    </p>
    ...
    <p className="text-gray-700 leading-relaxed mb-4">
      ...
      vigor imediatamente após sua publicação. O uso continuado da plataforma após as modificações
    ...
    <p className="text-gray-700 leading-relaxed mb-4">
      ...
      <strong>legal@sounativo.com</strong>
    </p>
  </div>
);
...
const PrivacyContent: React.FC = () => (
  <div className="prose prose-blue max-w-none text-gray-700">
    ...
    <p className="text-gray-700 leading-relaxed mb-6">
      No SouNativo, levamos sua privacidade a sério. Esta política descreve como coletamos, usamos,
    ...
    <p className="text-gray-700 leading-relaxed mb-4">Compartilhamos seus dados apenas com:</p>
    <ul className="list-disc pl-6 text-gray-700 mb-4 space-y-2">
    ...
    <p className="text-gray-700 leading-relaxed mb-4">
      Utilizamos cookies para melhorar sua experiência na plataforma, analisar o uso do site e
      personalizar conteúdo. Você pode gerenciar suas preferências de cookies nas configurações do
      seu navegador.
    </p>
    ...
    <p className="text-gray-700 leading-relaxed mb-4">
      ...
      significativas através do email cadastrado ou por aviso na plataforma.
    </p>
    ...
    <ul className="list-none pl-0 text-gray-700 mb-4 space-y-2">
      <li>
        <strong>Email:</strong> privacidade@sounativo.com
      </li>
      <li>
