...
const BroadcastSpotlight: React.FC<BroadcastSpotlightProps> = ({ className = '' }) => {
  ...
  const saveDismissedId = (id: string) => {
    ...
    localStorage.setItem(
      `broadcast_spotlight_dismissed_${user.id}`,
  ...
  const loadLatestNotification = async () => {
    ...
    try {
      ...
      const unreadNotifications = data.filter((n) => !readIds.has(n.id) && !dismissedIds.has(n.id));
  ...
  if (
    !user ||
    (user.role !== UserRole.CLIENT && user.role !== UserRole.AGENCY && user.role !== UserRole.GUIDE)
  ) {
    return null;
  }
  ...
  return (
    <div
      className={`relative overflow-hidden bg-white border border-stone-200 border-l-4 border-l-primary-500 rounded-xl shadow-sm ${className}`}
    >
      ...
      <div className="relative flex items-center gap-4 px-5 py-4">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-stone-900 truncate">{latestNotification.title}</p>
          <p className="text-sm text-stone-500 truncate">
            {latestNotification.message.substring(0, 80)}
            {latestNotification.message.length > 80 ? '...' : ''}
          </p>
        </div>
