  ...
  try {
    ...
    if (passengers.length > 0) {
      ...
      tableBody = passengers.map((p: any, index: number) => {
        ...
        logger.info(
          `PDF Table Row ${index + 1}: "${passengerName}" | "${formattedDoc}" | "${passengerType}"`
        );

        return [passengerName, formattedDoc, passengerType];
      });

      logger.info(`✅ PDF Generator: Created table with ${tableBody.length} row(s)`);
      logger.info(
        'Table body preview:',
        tableBody.map((row, idx) => `[${idx + 1}] ${row[0]} | ${row[1]} | ${row[2]}`)
      );
      ...
      tableBody = booking.passengerDetails.map((p: PassengerDetail) => {
        ...
        if (
          formattedDoc &&
          formattedDoc !== '---' &&
          /^\d{11}$/.test(formattedDoc.replace(/\D/g, ''))
        ) {
          formattedDoc = formattedDoc.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4');
        }
        return [p.name || '---', formattedDoc, getPassengerType(p)];
      });
    } else if (client) {
      ...
      tableBody = [[client.name || 'Cliente', formattedDoc, 'Adulto']];
    } else {
