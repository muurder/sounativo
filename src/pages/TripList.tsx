...
export const TripList: React.FC = () => {
  ...
  const initialTrips = isResolvingAgency
    ? []
    : currentAgency
      ? getAgencyPublicTrips(currentAgency.agencyId)
      : trips;
  ...
  const filteredTrips = useMemo(() => {
    ...
    if (adultsParam || childrenParam) {
      const totalGuests =
        (adultsParam ? parseInt(adultsParam) : 0) + (childrenParam ? parseInt(childrenParam) : 0);
      if (totalGuests > 0) {
        result = result.filter((t) => {
  ...
  if (isAgencyNotFound) {
    return (
      <div className="text-center py-20">
        ...
        <Link to="/agencies" className="text-primary-600 font-bold hover:underline">
          Voltar para lista de agências
        </Link>
      </div>
    );
  }
    ...
    <div className="space-y-8 pb-12">
              ...
              <HeroSearch
                ...
                onSearch={(params) => {
                  ...
                  if (params.dateRange.start)
                    newParams.set('startDate', params.dateRange.start.toISOString().split('T')[0]);
                  else newParams.delete('startDate');
                  if (params.dateRange.end)
                    newParams.set('endDate', params.dateRange.end.toISOString().split('T')[0]);
                  else newParams.delete('endDate');
                  if (params.guests.adults > 0)
                    newParams.set('adults', params.guests.adults.toString());
                  else newParams.delete('adults');
                  if (params.guests.children > 0)
                    newParams.set('children', params.guests.children.toString());
      ...
      <div className="flex flex-col md:flex-row gap-8 px-2">
        ...
        <aside
          ...
          fixed md:static inset-0 z-50 overflow-y-auto transition-transform duration-300 scrollbar-hide
        ...
        >
          ...
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 space-y-6 sticky top-24">
            ...
            <div className="border-b border-gray-100 pb-4">
              <button
                onClick={() => toggleAccordion('traveler')}
                className="w-full flex justify-between items-center font-bold text-gray-900 mb-3 hover:text-primary-600 transition-colors"
              >
                ...
                <div className="space-y-2">
                  {travelerOptions.map((type) => (
                    <label key={type.id} className="flex items-center cursor-pointer group">
                      <div
                        className={`w-5 h-5 rounded border flex items-center justify-center mr-3 transition-all ${selectedTravelerTypes.includes(type.id) ? 'bg-primary-600 border-primary-600' : 'border-gray-300 group-hover:border-primary-400 bg-white'}`}
                      >
                      ...
                      />
                      <span
                        className={`text-sm ${selectedTravelerTypes.includes(type.id) ? 'text-primary-700 font-bold' : 'text-gray-600'}`}
                      >
                        {type.label}
                      </span>
                    </label>
              ...
              {openFilterSections['duration'] && (
                <div className="space-y-2">
                  {durationOptions.map((opt) => (
                    ...
                    >
                      <div
                        className={`w-5 h-5 rounded border flex items-center justify-center mr-3 transition-all ${durationParam === opt.id ? 'bg-primary-600 border-primary-600' : 'border-gray-300 group-hover:border-primary-400 bg-white'}`}
                      >
                        {durationParam === opt.id && (
                          <div className="w-2 h-2 bg-white rounded-full" />
                        )}
                      </div>
                      <span
                        className={`text-sm ${durationParam === opt.id ? 'text-primary-700 font-bold' : 'text-gray-600'}`}
                      >
                        {opt.label}
                      </span>
                    </div>
              ...
              {openFilterSections['price'] && (
                <div className="space-y-2">
                  {priceOptions.map((opt) => (
                    ...
                    >
                      <div
                        className={`w-5 h-5 rounded border flex items-center justify-center mr-3 transition-all ${priceParam === opt.id ? 'bg-primary-600 border-primary-600' : 'border-gray-300 group-hover:border-primary-400 bg-white'}`}
                      >
                        {priceParam === opt.id && <div className="w-2 h-2 bg-white rounded-full" />}
                      </div>
                      <span
                        className={`text-sm ${priceParam === opt.id ? 'text-primary-700 font-bold' : 'text-gray-600'}`}
                      >
                        {opt.label}
                      </span>
                    </div>
            ...
            {(q || tagsParam || travelerParam || durationParam || priceParam || categoryParam) && (
              <div className="pt-6 border-t border-gray-100">
                <button
                  ...
                  className="w-full text-red-600 font-bold hover:underline py-2.5 bg-red-50 rounded-xl flex items-center justify-center gap-2"
                >
        ...
        <button
          ...
          className="md:hidden fixed bottom-24 right-4 z-50 bg-primary-600 text-white p-4 rounded-full shadow-lg flex items-center gap-2 font-bold hover:bg-primary-700 transition-all active:scale-95 animate-[scaleIn_0.5s]"
        >
          ...
          {filteredTrips.length > 0 ? (
            <>
              {viewMode === 'list' ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6 animate-[fadeInUp_0.5s] items-stretch">
                  {filteredTrips.map((trip) => (
                    <InteractiveTripCard key={trip.id} trip={trip} />
                  ))}
                </div>
              )}
            </>
            ...
            <div className="text-center py-20 bg-white rounded-xl border border-dashed border-gray-200 shadow-sm">
              ...
              <p className="text-gray-500 mb-6 max-w-md mx-auto">
                Não encontramos resultados para sua busca. Tente ajustar os filtros ou buscar por
