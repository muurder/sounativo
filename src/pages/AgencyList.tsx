...
const AgencyAvatar: React.FC<{ agency: Agency; className: string; fallbackClassName: string }> = ({
  agency,
  className,
  fallbackClassName,
}) => {
  ...
  // If there's no logo, or error, or it's just the external fallback, render the local CSS avatar immediately
  const isExternalFallback = agency.logo && agency.logo.includes('ui-avatars.com');

  if (!agency.logo || imgError || isExternalFallback) {
    return <div className={fallbackClassName}>{getInitials(agency.name)}</div>;
  }

  return (
    <div className="relative isolate">
      {/* Show the local CSS fallback instantly WHILE the real logo is loading */}
      {!isLoaded && (
        <div className={`absolute inset-0 z-10 ${fallbackClassName}`}>
      ...
      <img
        src={agency.logo}
        alt={agency.name}
        className={`${className} ${isLoaded ? 'opacity-100' : 'opacity-0'} transition-opacity duration-300 text-transparent relative z-20`}
          ...
          <div>
            <h3 className="font-bold text-gray-900 mb-4 flex items-center justify-between">
              <span className="flex items-center">
                <Filter size={16} className="mr-2" /> Especialidade
              </span>
        ...
        <div className="flex-1">
          <div className="mb-4 flex justify-between items-center">
            <div className="text-sm text-gray-500 font-medium">
              Mostrando <span className="text-gray-900 font-bold">{totalCount}</span> agências
            </div>
            {(selectedSpecialty || searchTerm) && (
              <button
                onClick={clearFilters}
            ...
            >
              {agencies.map((agency) => (
                <Link
                  key={agency.agencyId}
                  to={`/${agency.slug}`}
                  className={`bg-white border border-gray-100 rounded-3xl hover:shadow-xl transition-all duration-300 group relative flex animate-[fadeIn_0.3s] ${viewMode === 'grid' ? 'flex-col overflow-hidden h-full' : 'flex-row p-4 items-center'}`}
                >
                  {viewMode === 'grid' ? (
                    <>
                      {/* Header Banner/Cover */}
                      ...
                      <div className="px-6 pb-6 pt-2 flex-1 flex flex-col">
                        ...
                        <p className="text-gray-600 text-sm text-center line-clamp-3 mb-6 flex-grow">
                          {agency.description ||
                            'Agência especializada em experiências únicas de viagem.'}
                        </p>
                    ...
                    <>
                      <div className="flex items-center gap-6 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          <h3 className="font-bold text-gray-900 leading-tight group-hover:text-emerald-600 transition-colors text-lg truncate">
                            {agency.name}
                          </h3>
                          <div className="flex items-center text-sm text-gray-500 mt-1">
                            <MapPin size={12} className="mr-1 flex-shrink-0" /> <span>Brasil</span>
                          </div>
                        </div>
                      </div>
                      <div className="hidden md:block flex-1 px-4 border-l border-gray-100 ml-4 h-full">
                        <p className="text-gray-500 text-sm line-clamp-2">{agency.description}</p>
                      </div>
                      <div className="flex items-center gap-4 ml-auto border-l border-gray-100 pl-4">
                        <div className="text-sm font-bold text-emerald-600 group-hover:text-emerald-700 flex items-center whitespace-nowrap bg-emerald-50 px-4 py-2 rounded-full">
                          Ver Página{' '}
                          <ArrowRight
                            size={16}
