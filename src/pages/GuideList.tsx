...
const GuideList: React.FC = () => {
  ...
  const allGuides = useMemo(() => {
    return agencies.filter(
      (agency) => agency.isGuide || agency.customSettings?.tags?.includes('GUIA')
    );
  }, [agencies]);
  ...
  const filteredGuides = useMemo(() => {
    ...
    if (selectedSpecialty) {
      filtered = filtered.filter(
        (g) =>
          g.specialties?.includes(selectedSpecialty) ||
          g.customSettings?.tags?.includes(selectedSpecialty)
      );
    }
    return filtered;
  ...
  const allSpecialties = Array.from(
    new Set(
      allGuides
        .flatMap((g) => g.specialties || g.customSettings?.tags || [])
        .filter((t) => t !== 'GUIA')
    )
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="relative h-[300px] sm:h-[400px] md:h-[500px] flex items-center justify-center mb-8 sm:mb-12 overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          ...
          <h1 className="text-4xl md:text-6xl font-extrabold text-white mb-6 leading-tight drop-shadow-2xl">
            Encontre o{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary-200 to-primary-50">
              Talento Ideal
            </span>{' '}
            para sua aventura
          </h1>
            ...
            {paginatedGuides.length <= 2 && paginatedGuides.length > 0 && (
              <div className="bg-gradient-to-br from-stone-50 to-stone-100 rounded-3xl border-2 border-dashed border-stone-300 p-12 flex flex-col items-center justify-center text-center min-h-[400px] w-full">
                ...
                <h3 className="text-lg font-bold text-stone-700 mb-2">
                  Mais guias chegando em breve...
                </h3>
                <p className="text-stone-500 text-sm">
                  Estamos sempre adicionando novos talentos especializados.
                </p>
              </div>
            )}
...
const TalentCard: React.FC<{ guide: Agency; whatsappLink: string }> = ({ guide, whatsappLink }) => {
  ...
  const portfolio = guide.customSettings?.included?.length
    ? guide.customSettings.included
    : [
      ...
      <div className="relative h-32 bg-primary-800">
        ...
        <div className="absolute -bottom-12 left-6">
          <div className="relative">
            ...
            {guide.cadastur && (
              <div
                className="absolute -right-2 -bottom-2 bg-primary-700 text-white p-1.5 rounded-full border-2 border-white shadow-lg"
      ...
      <div className="pt-14 px-6 pb-6 flex-1 flex flex-col">
        <div className="mb-4">
          ...
          <div className="flex items-center text-gray-500 text-sm mt-2">
            ...
            {guide.address?.city || 'Localização não definida'} • {guide.experienceYears || '2'}{' '}
            anos de exp.
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {guide.languages?.map((lang) => (
            <span
              key={lang}
              className="inline-flex items-center px-3 py-1.5 bg-stone-100 text-stone-700 text-xs font-bold rounded-full border border-stone-200"
            >
        ...
        <div className="mt-auto flex gap-3">
          <Link
            to={`/${guide.slug}`}
            className="flex-1 py-3 bg-stone-100 hover:bg-stone-200 text-stone-800 font-bold rounded-full text-center text-sm transition-colors flex items-center justify-center border border-stone-200"
          >
            Ver Perfil
          </Link>
          <a
            ...
            className="flex-1 py-3 bg-secondary-500 hover:bg-secondary-600 text-white font-bold rounded-full text-center text-sm transition-all shadow-lg shadow-secondary-500/30 flex items-center justify-center"
          >
