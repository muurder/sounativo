...
export const CheckoutSuccess: React.FC = () => {
      ...
      <div className="min-h-[80vh] flex flex-col items-center justify-center text-center px-4 bg-gray-50">
        <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100 max-w-md w-full">
          ...
          <p className="text-gray-500 mb-6 text-sm">
            Os dados temporários da reserva não estão mais disponíveis aqui. Acesse seu painel para
            ver o voucher.
          </p>
          <Link
  ...
  const linkDashboard = agencySlug
    ? `/${agencySlug}/client/BOOKINGS`
  ...
  return (
    <div className="min-h-[80vh] flex flex-col items-center justify-center text-center px-4 py-12 bg-gray-50 animate-[fadeIn_0.5s]">
      ...
      <div className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-200">
        ...
        <div className="p-8 bg-white relative">
          ...
          <div className="absolute -top-3 left-0 w-full flex justify-between px-2">
            {[...Array(12)].map((_, i) => (
              <div key={i} className="w-4 h-4 bg-gray-50 rounded-full -mt-2"></div>
            ))}
          </div>
          ...
          <div className="space-y-6 mt-4">
            ...
            <div className="bg-gray-50 p-4 rounded-xl flex items-center gap-4">
              ...
              <div className="text-left">
                <p className="text-xs text-gray-500 leading-tight mb-1">
                  Apresente este código ou acesse seu voucher digital no painel.
                </p>
                <button
                  onClick={(e) => {
                    e.preventDefault();
            ...
            <div className="space-y-3 pt-2">
              <div className="flex gap-3">
                <Link
                  to={linkDashboard}
                  className="flex-1 bg-gray-900 text-white py-3 rounded-xl font-bold shadow-lg hover:bg-black transition-all flex justify-center items-center gap-2 group text-sm"
                >
                  <Ticket size={16} /> Minhas Viagens
                </Link>
                <button
                  onClick={handleShare}
                  className="bg-primary-50 text-primary-600 p-3 rounded-xl hover:bg-primary-100 transition-colors shadow-sm border border-primary-100"
                  title="Compartilhar"
                >
                  <Share2 size={20} />
                </button>
              ...
              <Link
                to={linkTrips}
                className="block w-full text-gray-500 py-2 rounded-xl font-bold text-xs hover:text-gray-700 transition-colors"
              >
      ...
      <p className="text-gray-400 text-xs mt-8 max-w-xs mx-auto leading-relaxed">
        Enviamos um e-mail com todos os detalhes do seu pedido. Em caso de dúvidas, entre em contato
        com a agência.
      </p>
    </div>
  );
