...
const AgencyDashboard: React.FC = () => {
  ...
  const handleDuplicateTrip = async (trip: Trip) => {
    ...
    try {
      ...
      showToast(
        `Erro ao duplicar ${tripLabelLower}: ${error.message || 'Erro desconhecido'}`,
        'error'
      );
    } finally {
  ...
  const confirmDeleteTrip = async () => {
    ...
    try {
      ...
      showToast(
        `Erro ao excluir ${tripLabelLower}: ${error.message || 'Erro desconhecido'}`,
        'error'
      );
    } finally {
  ...
  const toggleTripStatus = async (tripId: string) => {
    ...
    try {
      ...
      showToast(
        `${tripLabel} ${!tripToUpdate.is_active ? 'publicado' : 'pausado'} com sucesso.`,
        'success'
      );
  ...
  const handleConfirmSubscription = async () => {
    ...
    try {
      await updateUser({ subscriptionPlan: showConfirmSubscription.id as any });
      await refreshData();
      ...
      const message = encodeURIComponent(
        `Olá, acabei de assinar o plano ${showConfirmSubscription.name} para a agência ${currentAgency.name}. Gostaria de confirmar.`
      );
  ...
  const stats: DashboardStats = useMemo(() => {
    if (!myTrips || !myBookings)
      return { activeTrips: 0, totalSales: 0, totalViews: 0, conversionRate: 0, totalRevenue: 0 };
          ...
          <div className="p-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
              <div>
                <h2 className="text-2xl font-bold text-gray-900 mb-1">
                  Meu Plano:{' '}
                  <span className={isStarter ? 'text-gray-600' : 'text-primary-600'}>
                    {currentPlanObj.name}
                  </span>
                </h2>
              ...
              <div className="flex gap-3">
                {!isStarter && (
                  <>
                    <button
                      onClick={() => setShowPaymentManagementDialog(true)}
                      className="bg-white border border-gray-200 text-gray-700 font-bold py-2.5 px-5 rounded-xl hover:bg-gray-50 text-sm"
                    >
                      Gerenciar Pagamento
                    </button>
                    <button
                      onClick={() => setShowConfirmSubscription(currentPlanObj)}
                      className={`text-white font-bold py-2.5 px-5 rounded-xl text-sm ${planColor} hover:opacity-90`}
                    >
                      Renovar Agora
                    </button>
                  </>
            ...
            <div className="bg-gray-50 rounded-xl p-6 border border-gray-100 mb-8">
              <div className="flex justify-between items-center mb-2 text-sm font-bold text-gray-700">
                ...
                <span className={daysLeft < 7 ? 'text-red-500' : 'text-green-600'}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-gray-500 text-xs uppercase font-bold">Cobrança</p>
                    <p className="font-bold text-gray-900">
                      {isValidDate(expiryDate) ? expiryDate.toLocaleDateString() : 'N/A'}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-gray-500 text-xs uppercase font-bold">Valor</p>
                    <p className="font-bold text-gray-900">
                      {isStarter ? 'Grátis' : `R$ ${currentPlanObj.price.toFixed(2)}/mês`}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {PLANS.map((plan) => {
            ...
            return (
              <div
                key={plan.id}
                className={`rounded-2xl shadow-lg border p-8 relative ${isCurrent ? 'ring-2 ring-primary-500 bg-gray-50' : 'bg-white'} ${isPremium ? 'border-purple-200' : 'border-gray-200'}`}
              >
                {isPremium && (
                ...
                <div className="text-3xl font-extrabold mb-6">
                  {isStarterPlan ? 'Grátis' : `R$ ${plan.price}`}
                ...
                <ul className="space-y-3 mb-8">
                  {plan.features.map((f, i) => (
                    <li key={i} className="flex items-start text-sm text-gray-700">
                      <Check
                        size={16}
                        className={`mr-2 flex-shrink-0 mt-0.5 ${isPremium ? 'text-purple-500' : 'text-green-500'}`}
                      />{' '}
                      {f}
                    </li>
                  ))}
  ...
  return (
    <>
      ...
      <UpgradeModal
        ...
        currentPlan={currentAgency.subscriptionPlan as 'STARTER' | 'BASIC' | 'PREMIUM'}
        maxTrips={PLANS.find((p) => p.id === currentAgency.subscriptionPlan)?.maxTrips || 5}
        currentActiveTrips={stats.activeTrips || 0}
        onUpgrade={() => {
      ...
      <ConfirmDialog
        ...
        onConfirm={() =>
          window.open('https://wa.me/5511987697684?text=Quero%20gerenciar%20meu%20plano', '_blank')
        }
        title="Gerenciar Assinatura"
        message="Para alterar dados de pagamento ou cancelar, entre em contato com nosso suporte financeiro."
      ...
      <div className="max-w-[1600px] mx-auto py-6 px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              ...
              <div className="px-5 -mt-10">
                <div className="relative inline-block">
                  <img
                    src={
                      currentAgency?.logo ||
                      `https://ui-avatars.com/api/?name=${encodeURIComponent(currentAgency?.name || 'Agência')}&background=1a5d3a&color=fff&bold=true`
                    }
                ...
                {tabs
                  ...
                  .map((tab, index, arr) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-3.5 transition-all group relative ${
                        activeTab === tab.id ? 'bg-primary-50' : 'hover:bg-stone-50'
                    ...
                    >
                      ...
                      <div
                        className={`w-8 h-8 rounded-lg flex items-center justify-center transition-transform group-hover:scale-110 ${
                          activeTab === tab.id ? 'bg-primary-100' : 'bg-stone-100'
                        }`}
                      >
                        <tab.icon
                          size={16}
                          className={activeTab === tab.id ? 'text-primary-600' : 'text-stone-600'}
                        />
                      </div>
                      <span
                        className={`text-sm font-medium ${activeTab === tab.id ? 'text-primary-700' : 'text-stone-700'}`}
                      >
                        {tab.label}
                      </span>
                    </button>
            ...
            <div className="rounded-2xl overflow-hidden shadow-lg relative">
              ...
              <div className="relative p-5 text-white">
                ...
                <p className="text-xs text-white/90 mb-3 drop-shadow-sm">
                  Adicione um novo pacote ao seu catálogo
                </p>
                <button
          ...
          <div className="lg:col-span-3 space-y-6">
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    <div className="flex bg-gray-100 p-1 rounded-lg">
                      <button
                        ...
                        className={`p-1.5 rounded-md transition-all ${tripViewMode === 'GRID' ? 'bg-white shadow-sm text-primary-600' : 'text-gray-500'}`}
                      >
                        <LayoutGrid size={18} />
                      </button>
                      <button
                        onClick={() => setTripViewMode('TABLE')}
                        className={`p-1.5 rounded-md transition-all ${tripViewMode === 'TABLE' ? 'bg-white shadow-sm text-primary-600' : 'text-gray-500'}`}
                      >
                        <List size={18} />
                      </button>
                    </div>
                    <button
                      onClick={() => setShowCreateTrip(true)}
                      className="flex-1 sm:flex-none bg-primary-600 text-white px-4 py-2 rounded-lg font-bold text-sm hover:bg-primary-700 flex items-center justify-center gap-2"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredTrips.map((trip) => {
                      ...
                      return (
                        <div
                          key={trip.id}
                          className="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all group flex flex-col h-full"
                        >
                          <div className="relative h-48 bg-gray-100 overflow-hidden">
                            {tripToDisplay.images &&
                            tripToDisplay.images.length > 0 &&
                            tripToDisplay.images[0] ? (
                              <>
                                <img
                          ...
                          <div className="p-4 flex-1 flex flex-col">
                            <h3
                              ...
                              title={tripToDisplay.title}
                            >
                              {tripToDisplay.title}
                            </h3>
                            <div className="text-sm text-gray-500 mb-4 flex items-center gap-2">
                              <MapPin size={14} className="text-primary-500" />
                              <span className="truncate">{tripToDisplay.destination}</span>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-2 mb-4">
                              <div className="bg-gray-50 p-2 rounded-lg">
                                ...
                                <p className="text-xs font-bold">
                                  {formatDate(tripToDisplay.startDate)}
                                </p>
                              </div>
                              <div className="text-right bg-gray-50 p-2 rounded-lg">
                                ...
                                <p className="text-sm font-extrabold text-primary-600">
                                  R$ {tripToDisplay.price.toLocaleString('pt-BR')}
                                </p>
                              </div>
                            </div>
                            ...
                            <div className="mt-auto border-t border-gray-100 pt-3 space-y-2">
                              ...
                              <div className="flex gap-2">
                                <button
                                  onClick={() =>
                                    window.open(
                                      `/#/${currentAgency.slug}/viagem/${tripToDisplay.slug || tripToDisplay.id}`,
                                      '_blank'
                                    )
                                  }
                    ...
                    <div className="hidden md:block overflow-x-auto">
                      <table className="min-w-full divide-y divide-gray-100">
                        <thead className="bg-gray-50">
                          <tr>
                            <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase w-20">
                              Imagem
                            </th>
                          ...
                          <div key={trip.id} className="p-4 space-y-3">
                            <div className="flex gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start mb-1">
                                  ...
                                  <Badge
                                    color={trip.is_active ? 'green' : 'gray'}
                                  >
                                    {trip.is_active ? 'ATIVO' : 'RASCUNHO'}
                                  </Badge>
                                </div>
                            ...
                            <div className="flex items-center justify-between text-xs text-gray-600 bg-gray-50 p-2 rounded-lg">
                              <span className="flex items-center gap-1">
                                <Calendar size={12} /> {formatDate(tripToDisplay.startDate)}
                              </span>
            ...
            {activeTab === 'REVIEWS' && (
              <AgencyReviews reviews={agencyReviews} currentAgency={currentAgency} />
            )}
                ...
                <form onSubmit={handleSaveProfile} className="space-y-6">
                  <div className="flex items-center gap-6 mb-6">
                    <div className="relative w-24 h-24">
                      <img
                        src={profileForm.logo || 'https://placehold.co/100x100?text=LOGO'}
                        className="w-full h-full rounded-2xl object-cover border-2 border-dashed border-gray-300"
                      />
                      <label className="absolute -bottom-2 -right-2 bg-primary-600 text-white p-2 rounded-full cursor-pointer hover:bg-primary-700 shadow-sm">
                        <Upload size={14} />
                        <input
                          type="file"
                          className="hidden"
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-500">
                        Faça upload de uma logo quadrada para melhor visualização.
                      </p>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <div className="flex">
                        <span className="bg-gray-100 px-3 py-2 border border-r-0 rounded-l-lg text-gray-500 text-sm flex items-center">
                          sounativo.com/
                        </span>
                        <input
                          value={profileForm.slug || ''}
                          onChange={(e) =>
                            setProfileForm({ ...profileForm, slug: slugify(e.target.value) })
                          }
                          className="flex-1 p-2.5 border rounded-r-lg"
                        />
                      </div>
                    </div>
                  </div>
