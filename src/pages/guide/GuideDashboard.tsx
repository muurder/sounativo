...
const MOCK_JOBS = [
  {
    id: 1,
    title: 'Guia para Grupo Corporativo',
    date: '2024-12-20',
    location: 'Capitólio, MG',
    agency: 'SouNativo Official',
    budget: 'R$ 600,00',
  },
  {
    id: 2,
    title: 'Transfer e City Tour',
    date: '2024-12-22',
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 hidden md:block">
              Gerencie sua carreira e encontre oportunidades
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="hidden md:flex flex-col items-end mr-2">
            ...
            <span className="text-xs text-green-600 flex items-center gap-1">
              <CheckCircle size={10} /> Cadastur Ativo
            </span>
          </div>
          <img
            src={(user as Agency).logo || `https://ui-avatars.com/api/?name=${user.name}`}
        ...
        <div className="hidden md:flex gap-4 mb-8 border-b border-gray-200 pb-1">
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 border-b-2 font-bold transition-colors ${
                activeTab === tab.id
              ...
              <div className="flex items-start gap-4 p-4 bg-blue-50 text-blue-800 rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-sm mt-1">
                    Guias com pelo menos 3 fotos reais no portfólio recebem 40% mais contatos.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-fade-in">
            <div className="p-6 border-b bg-gray-50 flex justify-between items-center">
              <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                <ImageIcon size={20} /> Editar Portfólio
              </h2>
              <button
                onClick={handleSaveProfile}
                className="flex items-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-lg font-bold hover:bg-primary-700 transition-colors"
              >
            ...
            <div className="p-6 md:p-8 space-y-8">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-2 text-right">
                  Mínimo 100 caracteres para melhor relevância.
                </p>
              </div>
                ...
                <div>
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <input
                      type="checkbox"
                      id="libras"
                      checked={languages.includes('LIBRAS')}
                      onChange={(e) => {
                    ...
                    />
                    <label htmlFor="libras" className="text-sm font-medium text-gray-700">
                      Atendo em LIBRAS (Língua Brasileira de Sinais)
                    </label>
                  </div>
                </div>
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
                  <Camera size={16} /> Fotos do Portfólio
                </label>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {portfolioImages.map((img, idx) => (
                    <div
                      key={idx}
                      className="aspect-video bg-gray-100 rounded-xl relative border-2 border-dashed border-gray-300 flex items-center justify-center group overflow-hidden"
                    >
                      {img ? (
                        <>
                ...
                <p className="text-xs text-gray-400 mt-2">
                  Use URLs de imagens públicas (Unsplash, Instagram CDN, etc) por enquanto.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {MOCK_JOBS.map((job) => (
                ...
                >
                  ...
                  <button
                    onClick={() => showToast('Candidatura enviada!', 'success')}
                    className="w-full py-2.5 rounded-xl border-2 border-primary-600 text-primary-600 font-bold hover:bg-primary-50 transition-colors"
                  >
                    Candidatar-se
                  </button>
                </div>
...
const StatCard: React.FC<{
  ...
  icon: any;
  color: string;
}> = ({ title, value, change, sub, icon: Icon, color }) => {
  const colors = {
    ...
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-start justify-between">
      <div>
        ...
        <div className="flex items-baseline gap-2">
          ...
          {change && (
            <span className="text-xs font-bold text-green-500 bg-green-50 px-1.5 py-0.5 rounded-md flex items-center">
              <TrendingUp size={10} className="mr-0.5" /> {change}
            </span>
          )}
        </div>
