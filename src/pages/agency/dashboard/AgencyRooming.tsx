...
export const RoomingManager: React.FC<RoomingManagerProps> = ({
  trip,
  bookings,
  clients,
  onSave,
  ...
  const [nameOverrides, setNameOverrides] = useState<Record<string, string>>(
    (trip.operationalData as any)?.passengerNameOverrides || {}
  );
  ...
  useEffect(() => {
    if (trip.operationalData?.hotels && trip.operationalData.hotels.length > 0) {
      ...
      // Default empty state
      const defaultHotel: HotelInstance = {
        id: `h-${Date.now()}`,
        name: 'Hotel Principal',
        rooms: [],
      };
    ...
    const fetchPassengers = async () => {
      try {
        ...
        const validBookings = bookings.filter(
          (b) => b.tripId && String(b.tripId) === String(trip.id)
        );
  ...
  const handleAssign = (
    roomId: string,
    passenger: { id: string; name: string; bookingId: string; details?: any }
  ) => {
  ...
  const saveRenameHotel = () => {
    ...
    const updated = hotels.map((h) =>
      h.id === editHotelNameId ? { ...h, name: tempHotelName } : h
    );
    saveHotels(updated);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-white">
      <ConfirmDialog
        ...
        message="Tem certeza que deseja excluir este quarto? Os passageiros voltarão para a lista."
        variant="danger"
      />
      <ConfirmDialog
        ...
        message={`Remover ${guestToRemove?.guestName} deste quarto?`}
        variant="warning"
        confirmText="Remover"
      />
      <ConfirmDialog
        ...
        title="Remover Hotel"
        message="Tem certeza que deseja remover este hotel? Todos os quartos e alocações serão perdidos."
        variant="danger"
        confirmText="Remover"
      />
      ...
      <div className="bg-slate-50 border-b p-4 flex flex-col gap-4 flex-shrink-0">
        <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide">
            ...
            >
              {editHotelNameId === hotel.id ? (
                ...
                <div className="flex items-center gap-2 w-full">
                  <Building
                    size={14}
                    className={activeHotelId === hotel.id ? 'text-primary-500' : 'text-gray-400'}
                  />
                  ...
                  {activeHotelId === hotel.id && (
                    <div className="flex items-center gap-1 ml-auto opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          startRenameHotel(hotel.id, hotel.name);
                        }}
          ...
          <button
            ...
            className="p-2 rounded-lg bg-white border border-dashed border-gray-300 text-gray-400 hover:text-primary-600 hover:border-primary-300 transition-colors"
          >
            <Plus size={16} />
          </button>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {invType === 'COLLECTIVE' && (
              <div className="relative animate-[fadeIn_0.2s]">
                <input
                  ...
                  value={invCustomCap}
                  onChange={(e) =>
                    setInvCustomCap(e.target.value === '' ? '' : parseInt(e.target.value))
                  }
            ...
            <div className="flex items-center bg-white rounded-lg border border-gray-200 p-0.5">
              <button
                onClick={() => setInvQty(Math.max(1, invQty - 1))}
                className="px-2 py-1 text-gray-500 hover:bg-gray-100 rounded-l-md font-bold"
              >
                -
              </button>
              <span className="px-2 text-xs font-bold min-w-[20px] text-center">{invQty}</span>
              <button
                onClick={() => setInvQty(invQty + 1)}
                className="px-2 py-1 text-gray-500 hover:bg-gray-100 rounded-r-md font-bold"
              >
                +
              </button>
            </div>
            ...
            <button
              ...
              className="bg-primary-600 text-white px-4 py-1.5 rounded-lg text-xs font-bold hover:bg-primary-700 flex items-center gap-1 shadow-sm ml-2"
            >
              <Plus size={14} /> Criar
            </button>
          </div>
          ...
          <div className="flex gap-4 text-xs text-gray-600 font-medium bg-white px-4 py-2 rounded-full border shadow-sm">
            ...
            <span>
              Ocupado: <b className="text-blue-600">{occupied}</b>
            </span>
            <span>
              Livre: <b className="text-green-600">{totalCap - occupied}</b>
            </span>
          </div>
        </div>
      </div>
          ...
          <div className="p-4 border-b bg-gray-50 space-y-3">
            <div className="flex justify-between items-center">
              <h4 className="font-bold text-gray-900 text-sm flex items-center gap-2">
                <Users size={16} /> Passageiros ({assignedCount}/{allPassengers.length})
              </h4>
              <button
            ...
            {filteredPassengers.map((p) => {
              ...
              return (
                    ...
                    if (!assignedInfo) {
                      setSelectedPassenger(
                        selectedPassenger?.id === p.id
                          ? null
                          : { id: p.id, name: displayName, bookingId: p.bookingId }
                      );
                    }
                  }}
                  className={`
                ...
                >
                  <div className="flex items-center gap-3 overflow-hidden">
                    {!assignedInfo && (
                      <Grip
                        size={12}
                        className={
                          selectedPassenger?.id === p.id ? 'text-white/50' : 'text-gray-300'
                        }
                      />
                    )}
                    {assignedInfo && (
                    ...
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${isSelected ? 'bg-white/20' : 'bg-gray-100 text-gray-500'}`}
                    >
                      {displayInitial}
                    </div>
                    <div className={`min-w-0 flex flex-col`}>
                      ...
                      <div
                        className={`flex items-center text-xs mt-0.5 ${selectedPassenger?.id === p.id ? 'text-white/80' : 'text-gray-500'}`}
                      >
                        {p.isManual ? (
                          <>
                  ...
                  {assignedInfo && (
                    <span
                      className="text-[10px] font-bold bg-blue-50 text-blue-600 px-2 py-0.5 rounded border border-blue-100 truncate max-w-[80px]"
                      title={assignedInfo}
                    >
                      {assignedInfo.split(' - ').pop()}
                    </span>
                  )}
                </div>
              );
        ...
        <div className="flex-1 bg-slate-100 overflow-y-auto p-6 custom-scrollbar">
          {activeHotel ? (
              ...
              {activeHotel.rooms.map((room) => {
                ...
                return (
                  <div
                    ...
                    onDragOver={(e) => {
                      e.preventDefault();
                      if (!isFull) setDragOverRoom(room.id);
                    }}
                  ...
                  >
                    <div className="p-3 border-b flex justify-between items-center bg-gray-50/50 w-full">
                      <div className="flex items-center gap-3 flex-1 min-w-0 truncate">
                        <div
                          className={`p-2 rounded-lg ${isFull ? 'bg-green-100 text-green-600' : 'bg-blue-50 text-blue-600'}`}
                        >
                          <BedDouble size={18} />
                        </div>
                        <div className="truncate flex-1">
                          ...
                          <div className="flex items-center gap-2 text-[10px] font-bold text-gray-500 uppercase">
                            <span>{room.type}</span>
                            <span className={isFull ? 'text-green-600' : 'text-blue-600'}>
                      ...
                      <button
                        ...
                        className="text-gray-300 hover:text-red-500 p-1.5 flex-shrink-0"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
                    <div className="p-3 space-y-2 min-h-[80px]">
                      {room.guests.map((g) => (
                        <div
                          key={g.bookingId}
                          className="bg-blue-50/50 px-3 py-2 rounded text-xs font-medium flex justify-between items-center group/guest border border-blue-100/50"
                        >
                          ...
                          <button
                            ...
                            className="text-blue-300 hover:text-red-500 opacity-0 group-hover/guest:opacity-100"
                          >
                            <X size={14} />
                          </button>
                        </div>
                      ))}
                      {Array.from({ length: Math.max(0, room.capacity - room.guests.length) }).map(
                        (_, i) => (
                          <div
                            key={i}
                            className={`border-2 border-dashed rounded px-3 py-2 text-xs flex items-center justify-center gap-1 select-none ${isTarget ? 'border-primary-200 bg-primary-50 text-primary-600 font-bold' : 'border-gray-100 text-gray-300'}`}
                          >
                            {isTarget ? (
                              <>
                                <MousePointer2 size={12} /> Alocar Aqui
                              </>
                            ) : (
                              'Vaga Livre'
                            )}
            ...
            <div className="flex items-center justify-center h-full text-gray-400 text-sm">
              Selecione ou adicione um hotel para gerenciar os quartos.
            </div>
          )}
        </div>
      </div>
