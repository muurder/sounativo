...
const RecentBookingsTable: React.FC<RecentBookingsTableProps> = ({ bookings, clients }) => {
  ...
  const generatePDF = async (booking: Booking) => {
    ...
    try {
      ...
      doc.text('Emitido por SouNativo - O maior marketplace de viagens do Brasil.', 105, y, {
        align: 'center',
      });
          ...
          >
              ...
              {clients.find((c) => c.id === selectedBooking.clientId) && (
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <p className="text-base font-bold text-gray-900">
                    {clients.find((c) => c.id === selectedBooking.clientId)?.name || 'N/A'}
                  </p>
                </div>
              )}
            </div>
            <div className="border-t border-gray-200 p-6 bg-gray-50 flex items-center justify-end gap-3">
              ...
              <button
                ...
                className="px-6 py-2.5 bg-primary-600 text-white rounded-lg font-bold hover:bg-primary-700 transition-colors flex items-center gap-2"
              >
