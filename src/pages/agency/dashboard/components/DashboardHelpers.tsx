...
export const Badge: React.FC<{
  ...
  return (
    <span
      className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${colors[color]} inline-flex items-center gap-1.5 w-fit`}
    >
      {children}
    </span>
  );
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-4">
        <div
          className={`p-3 rounded-xl ${bgColors[color]} group-hover:scale-105 transition-transform`}
        >
          <Icon size={24} />
        </div>
      </div>
              ...
              {actions.map((action, idx) => (
                ...
                >
                  {action.isLoading ? (
                    ...
                    <action.icon
                      size={18}
                      className={action.variant === 'danger' ? 'text-red-500' : 'text-primary-600'}
                    />
                  )}
...
export const NavButton: React.FC<{
...
}> = ({ tabId, label, icon: Icon, activeTab, onClick, hasNotification }) => (
  <button
  ...
  >
    <Icon
      size={20}
      className={`transition-colors ${activeTab === tabId ? 'text-primary-600' : 'text-gray-400 group-hover:text-gray-600'}`}
    />
