...
export const OperationsModule: React.FC<OperationsModuleProps> = ({
  ...
  const filteredTrips = myTrips.filter(
    (t) => t.is_active && t.title.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const generateManifest = async () => {
    ...
    try {
      ...
      doc.text(
        `Data: ${safeDate(selectedTrip.startDate)} - ${safeDate(selectedTrip.endDate)}`,
        15,
        70
      );
      ...
      const tripBookings = myBookings.filter(
        (b) => b.tripId && String(b.tripId) === String(selectedTrip.id) && b.status === 'CONFIRMED'
      );
      const totalPax =
        (opData.manualPassengers?.length || 0) +
        tripBookings.reduce((sum, b) => sum + b.passengers, 0);
        ...
        if (dbPassenger) {
          return {
            ...
            document: dbPassenger.cpf
              ? dbPassenger.cpf.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4')
              : '-',
            phone: dbPassenger.whatsapp || '-',
            birthDate: dbPassenger.birth_date
              ? new Date(dbPassenger.birth_date).toLocaleDateString('pt-BR')
              : '-',
      ...
      tripBookings.forEach((b) => {
        const clientName =
          (b as any)._client?.name || clients.find((c) => c.id === b.clientId)?.name || 'Cliente';
      ...
      allPax.forEach((pax, index) => {
        ...
        doc.setTextColor(255, 255, 255);
        doc.setFontSize(6);
        doc.text(getInitials(pax.name), xOffset + colWidths[0] / 2, yPos + rowHeight / 2 + 1.5, {
          align: 'center',
        });
        doc.setTextColor(0, 0, 0);
        xOffset += colWidths[0];
        ...
        doc.setFontSize(7);
        doc.text(
          pax.name.length > 30 ? pax.name.substring(0, 28) + '...' : pax.name,
          xOffset + 2,
          yPos + 5.5
        );
        xOffset += colWidths[1];
        ...
        xOffset += colWidths[2];
        doc.text(
          pax.seat.length > 18 ? pax.seat.substring(0, 16) + '...' : pax.seat,
          17 + colWidths[0] + colWidths[1],
          yPos + 5.5
        );
        doc.text(pax.document, 17 + colWidths[0] + colWidths[1] + colWidths[2], yPos + 5.5);
        doc.text(
          pax.birthDate !== '-' ? new Date(pax.birthDate).toLocaleDateString('pt-BR') : '-',
          17 + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3],
          yPos + 5.5
        );
        doc.text(
          pax.type,
          17 + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] + colWidths[4],
          yPos + 5.5
        );
      ...
      if (opData.transport?.vehicles && opData.transport.vehicles.length > 0) {
        opData.transport.vehicles.forEach((vehicle) => {
          ...
          for (let r = 1; r <= rows; r++) {
            ...
            for (let c = 1; c <= cols; c++) {
              ...
              if (seatNum <= totalSeats) {
                ...
                doc.setFontSize(6);
                doc.text(seatNum.toString(), currentX + seatSize / 2, currentY + seatSize / 2 + 1, {
                  align: 'center',
                });
                ...
                if (occupant) {
                  ...
                  doc.text(displayName, currentX + seatSize / 2, currentY + seatSize + 3, {
                    align: 'center',
                  });
                }
          ...
          if (occupiedSeats.length > 0) {
            ...
            const sortedSeats = [...occupiedSeats].sort(
            ...
            sortedSeats.forEach((seat, idx) => {
              ...
              const dbPassenger = bookingPassengersMap.get(
                seat.bookingId.split('-')[0] +
                  '-' +
                  (seat.bookingId.includes('-') ? seat.bookingId.split('-')[1] : '0')
              );
        ...
        for (let r = 1; r <= rows; r++) {
          ...
          for (let c = 1; c <= cols; c++) {
            const seatNum = (r - 1) * cols + c;
            if (seatNum <= totalSeats) {
              const occupant = opData.transport.seats?.find(
                (s: PassengerSeat) => s.seatNumber === seatNum.toString()
              );
            ...
            const guestNames =
              room.guests && room.guests.length > 0
                ? room.guests
                    ...
                    .join(', ')
                : 'Vazio';
            const truncatedGuests =
              guestNames.length > 40 ? guestNames.substring(0, 38) + '...' : guestNames;
  ...
  return (
    <div className="flex h-full overflow-hidden bg-white relative">
      ...
      >
        <div
          className={`min-w-[20rem] h-full flex flex-col transition-opacity duration-300 ${isSidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        >
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4
                          className={`font-bold text-sm mb-1 line-clamp-2 ${selectedTripId === trip.id ? 'text-primary-700' : 'text-gray-800'}`}
                        >
                          {trip.title}
                        </h4>
              ...
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  <h2 className="text-sm md:text-lg font-bold text-gray-900 flex items-center gap-2 line-clamp-1">
                    {selectedTrip.title}
                    <span className="bg-blue-100 text-blue-700 text-[10px] px-2 py-0.5 rounded-full font-bold uppercase hidden md:inline-block">
                      Ativa
                    </span>
                  </h2>
                  {selectedTrip.destination && (
                    <p className="text-xs text-gray-500 hidden md:block line-clamp-1">
                      {selectedTrip.destination}
                    </p>
                  )}
                </div>
          ...
          {selectedTrip && (
            <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
              <div className="flex bg-gray-100 p-1 rounded-lg">
                ...
                >
                  <Bus size={14} className="inline mr-1 mb-0.5" />{' '}
                ...
                >
                  <BedDouble size={14} className="inline mr-1 mb-0.5" />{' '}
