  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 animate-[fadeIn_0.3s]">
      <h2 className="text-2xl font-bold text-gray-900 mb-6">
        Avaliações da Minha Agência ({reviews.length})
      </h2>
      {reviews.length > 0 ? (
            ...
            <div key={review.id} className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-gray-900 text-sm">
                      {review.clientName || 'Cliente SouNativo'}
                    </p>
              ...
              {review.tripTitle && (
                <p className="text-xs text-gray-500 mb-3">
                  Avaliação do pacote: <span className="font-bold">{review.tripTitle}</span>
                </p>
              )}
                ...
                <div className="flex-1 w-full relative group/reply">
                  {review.reply || review.response ? (
                    <div className="bg-gray-50 p-3 rounded-lg mt-3 border-l-4 border-primary-500 relative animate-[fadeIn_0.3s]">
                      ...
                      <button
                        onClick={() => {
                          setReplyText(review.reply || review.response || '');
                          setActiveReplyId(review.id);
                        }}
                        className="absolute top-2 right-2 text-gray-400 hover:text-primary-600 opacity-0 group-hover/reply:opacity-100 transition-opacity"
                      >
                        <Edit size={14} />
                      </button>
                    </div>
                    ...
                    <div className="bg-white p-3 rounded-lg border border-primary-200 shadow-sm w-full">
                      ...
                      <div className="flex justify-end gap-2 mt-2">
                        <button
                          ...
                          className="text-xs text-gray-500 font-bold hover:bg-gray-100 px-3 py-1.5 rounded"
                        >
                          Cancelar
                        </button>
                        <button
                          onClick={() => handleSendReply(review.id)}
                          className="text-xs bg-primary-600 text-white font-bold px-4 py-1.5 rounded hover:bg-primary-700 flex items-center gap-1"
                        >
                          <Send size={12} /> Enviar
                        </button>
                      </div>
                    ...
                    <button
                      ...
                      className="text-sm text-gray-500 hover:text-primary-600 font-medium flex items-center gap-2 py-1.5"
                    >
