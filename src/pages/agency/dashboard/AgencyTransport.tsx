...
export const TransportManager: React.FC<TransportManagerProps> = ({
  ...
  const [manualPassengers, setManualPassengers] = useState<ManualPassenger[]>(
    trip.operationalData?.manualPassengers || []
  );
  ...
  useEffect(() => {
    ...
    if (dbPassengers.size > 0) {
      ...
      console.log(
        `[Transport] ✅ ${namesCount} nomes carregados do banco de ${dbPassengers.size} passageiros`
      );
    }
  }, [dbPassengers]);
  ...
  const allPassengers = useMemo(() => {
    ...
    const booked = validBookings
      .filter((b) => b.status === 'CONFIRMED')
      .flatMap((b) => {
        const clientName =
          (b as any)._client?.name || clients.find((c) => c.id === b.clientId)?.name;
        return Array.from({ length: b.passengers }).map((_, i) => {
          ...
          if (i === 0) {
            ...
            finalName =
              ...
              detailName ||
              nameOverrides[id] ||
              `Acompanhante ${i} de ${clientName || 'Cliente'}`;
          }
  ...
  const isSeatOccupied = (seatNum: string) =>
    activeVehicle?.seats.find((s) => s.seatNumber === seatNum);
  ...
  const handleAssign = (
    seatNum: string,
    passenger: { id: string; name: string; bookingId: string }
  ) => {
  ...
  const handleAutoFill = async () => {
    ...
    try {
      ...
      const newSeats: PassengerSeat[] = [
        ...cleanedVehicles.find((v) => v.id === activeVehicle.id)!.seats,
      ];
      ...
      sortedGroups.forEach((group) => {
        ...
        group.forEach((passenger, idx) => {
          ...
          if (foundConsecutive && startIndex + idx < availableSeats.length) {
            ...
            while (
              availableIndex < availableSeats.length &&
              usedSeats.has(availableSeats[availableIndex])
            ) {
              availableIndex++;
            }
      ...
      const newlyAssigned =
        newSeats.length - cleanedVehicles.find((v) => v.id === activeVehicle.id)!.seats.length;
  ...
  const handleSaveCustomVehicle = (e: React.FormEvent) => {
    ...
    if (!editingVehicleId) {
      showToast(
        `Veículo "${customVehicleData.label || `Veículo ${vehicles.length + 1}`}" criado com sucesso!`,
        'success'
      );
    } else {
  ...
  const getPassengerDetails = (seat: PassengerSeat) => {
    ...
    if (!passenger) {
      passenger = allPassengers.find(
        (p) => p.id === seat.bookingId || p.name === seat.passengerName
      );
    }
    ...
    const booking = bookings.find(
      (b) => b.id === passenger.bookingId && b.tripId && String(b.tripId) === String(trip.id)
    );
    const client = booking
      ? (booking as any)._client || clients.find((c) => c.id === booking.clientId)
      : null;

    const finalName =
      dbPassenger?.full_name ||
      (passenger as any).details?.name ||
      seat.passengerName ||
      passenger.name ||
      'Passageiro (Sem nome)';
    ...
    return {
      name: finalName,
      avatar:
        client?.avatar || dbPassenger?.avatar || (passenger as any).details?.avatar || undefined,
      status: passenger.isMain
        ? 'Titular'
        : passenger.isAccompaniment
          ? `Acompanhante ${passenger.passengerIndex}`
          : passenger.isManual
            ? 'Manual'
            : 'Desconhecido',
    };
          ...
          {!showCustomVehicleForm && !isVehicleMenuOpen ? (
            <>
              ...
              <p className="text-gray-500 mb-6">
                Escolha o tipo de transporte para começar a organizar sua frota.
              </p>
                ...
                {Object.values(VEHICLE_TYPES).map((v) => (
                  ...
                  >
                    ...
                    <span className="text-xs text-gray-400 mt-1">
                      {v.totalSeats > 0 ? `${v.totalSeats} Lugares` : 'Definir'}
                    </span>
                  </button>
                ))}
              </div>
            ...
            <form onSubmit={handleSaveCustomVehicle} className="text-left max-w-sm mx-auto">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-gray-900">
                  {editingVehicleId ? 'Editar Veículo' : 'Veículo Personalizado'}
                </h3>
                <button type="button" onClick={() => setShowCustomVehicleForm(false)}>
                  <X size={20} />
                </button>
              </div>
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCustomVehicleData({ ...customVehicleData, label: e.target.value })
                    }
                    className="w-full border p-2 rounded"
                  />
                </div>
                <div>
                  <label className="text-sm font-bold">Lugares</label>
                  <input
                    type="number"
                    required
                ...
                <div>
                  <label className="text-sm font-bold">Colunas</label>
                  <select
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full overflow-hidden bg-white">
      <ConfirmDialog
        ...
        message={`Remover ${seatToDelete?.name} do assento ${seatToDelete?.seatNum}?`}
        variant="warning"
      />
      <ConfirmDialog
        isOpen={showClearSeatsModal}
        onClose={() => setShowClearSeatsModal(false)}
        onConfirm={confirmClearSeats}
        title="Limpar Todos os Assentos"
        message="Tem certeza que deseja limpar todos os assentos deste veículo? Esta ação não pode ser desfeita."
        variant="warning"
        confirmText="Limpar"
      />
      <ConfirmDialog
        ...
        message="Tem certeza que deseja remover este veículo? Todos os passageiros serão desvinculados."
        variant="danger"
        confirmText="Remover"
      />
      <ConfirmDialog
        ...
        message="Tem certeza que deseja remover este passageiro da lista? Ele será desvinculado de qualquer assento atribuído."
        variant="danger"
        confirmText="Remover"
      />
      ...
      <aside className="w-full lg:w-80 border-r border-gray-200 bg-white flex flex-col h-full shadow-sm z-10 flex-shrink-0">
        <div className="p-4 border-b bg-gray-50 space-y-3">
          <div className="flex justify-between items-center">
            <h4 className="font-bold text-gray-900 text-sm flex items-center gap-2">
              <User size={16} /> Passageiros ({assignedCount}/{allPassengers.length})
            </h4>
            <button
        ...
        <div className="p-4 flex-1 overflow-y-auto custom-scrollbar space-y-2">
          {showManualForm && (
            <ManualPassengerForm onAdd={handleAddManual} onClose={() => setShowManualForm(false)} />
          )}
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 overflow-hidden pr-6">
                  {!isAssigned && (
                    <Grip size={12} className={isSelected ? 'text-white/50' : 'text-gray-300'} />
                  )}
                  {isAssigned && <CheckCircle size={14} className="text-green-600 flex-shrink-0" />}
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${isSelected ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-500'}`}
                  >
                    {displayInitial}
                  </div>
                ...
                {isAssigned && (
                  <span className="text-[10px] font-bold text-green-700 bg-green-100 px-1.5 py-0.5 rounded ml-2 whitespace-nowrap truncate max-w-[80px]">
                    {assignedInfo}
                  </span>
                )}
            ...
            <div className="border-t pt-4 mt-4">
              ...
              <div className="space-y-1">
                {deletedPassengers.map((dp) => (
                  <div
                    key={dp.id}
                    className="p-2 bg-red-50 border border-red-100 rounded-lg flex justify-between items-center group hover:bg-red-100 transition-colors"
                  >
        ...
        <header className="relative flex items-center bg-white border-b border-gray-200 px-4 py-2 gap-2 overflow-x-auto scrollbar-hide flex-shrink-0">
          {vehicles.map((vehicle) => (
            <div
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-t-lg border-b-2 cursor-pointer transition-all min-w-[120px] justify-between group ${activeVehicleId === vehicle.id ? 'border-primary-500 text-primary-600 bg-primary-50 font-bold' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}
            >
              <span className="truncate max-w-[100px] text-xs">
                {vehicle.name}{' '}
                <span className="opacity-50 font-normal">({vehicle.config.totalSeats})</span>
              </span>
          ...
          <div className="relative z-[100]">
            <button
              ...
              className="flex items-center gap-1 px-3 py-2 text-xs font-bold rounded-lg bg-primary-50 text-primary-600"
            >
                  ...
                  {Object.values(VEHICLE_TYPES)
                    ...
                    .map((v) => (
                      <button
                        key={v.type}
                        onClick={() => handleSelectVehicleType(v.type)}
                        className="w-full text-left px-3 py-2 hover:bg-gray-50 text-sm text-gray-700 rounded-lg flex items-center gap-2"
                      >
                        <Truck size={14} /> {v.label}
                      </button>
                    ))}
                  <button
                    onClick={() => handleSelectVehicleType('CUSTOM')}
                    className="w-full text-left px-3 py-2 hover:bg-gray-50 text-sm text-primary-600 font-bold rounded-lg flex items-center gap-2"
                  >
                    <Settings2 size={14} /> Personalizado...
                  </button>
                </div>
              </>
            )}
        ...
        {activeVehicle && vehicles.length > 0 && (
          <div className="flex items-center gap-2 px-4 py-2 bg-white border-b border-gray-200 flex-shrink-0">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg text-xs font-bold hover:bg-green-700"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg text-xs font-bold hover:bg-red-700"
            >
            ...
            <>
              <div className="absolute top-4 right-4 z-20 flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg shadow-lg hover:bg-primary-700 text-xs font-bold"
                >
      ...
      {showCustomVehicleForm && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/50 backdrop-blur-sm p-0 md:p-4">
          <div className="bg-white rounded-t-2xl md:rounded-2xl p-6 w-full md:max-w-sm shadow-2xl relative">
            <button
              onClick={() => setShowCustomVehicleForm(false)}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value={3}>3 (Exec)</option>
                    <option value={4}>4 (Padrão)</option>
                  </select>
                </div>
              </div>
              <button className="w-full bg-primary-600 text-white py-3 rounded-lg font-bold">
                {editingVehicleId ? 'Salvar Alterações' : 'Criar Veículo'}
              </button>
            </form>
          </div>
        </div>
