...
export const HireGuides: React.FC = () => {
  ...
  const filteredGuides = useMemo(() => {
    ...
    if (selectedRegion) {
      filtered = filtered.filter(
        (g) =>
          g.address?.state?.includes(selectedRegion) || g.address?.city?.includes(selectedRegion)
      );
    }
    return filtered;
  ...
  const allLanguages = useMemo(
    () => Array.from(new Set(allGuides.flatMap((g) => g.languages || []))),
    [allGuides]
  );
  const allRegions = useMemo(
    () => Array.from(new Set(allGuides.map((g) => g.address?.state).filter(Boolean))),
    [allGuides]
  );
    ...
    <div className="p-4 md:p-8 pb-24 md:pb-8 animate-[fadeIn_0.3s]">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-lg">
          Profissionais verificados para elevar a experiência dos seus pacotes.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredGuides.map((guide) => (
          <div
            key={guide.id}
            className="bg-white rounded-2xl shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] border border-gray-100 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-col overflow-hidden group"
          >
            <div className="p-6 flex flex-col items-center text-center border-b border-gray-50">
              ...
              <div className="flex items-center text-gray-500 text-xs mb-3">
                ...
                {guide.address?.city || 'Brasil'}{' '}
                {guide.address?.state ? `- ${guide.address.state}` : ''}
              </div>
              ...
              <div className="flex flex-wrap justify-center gap-1.5 w-full">
                {guide.languages?.slice(0, 3).map((lang) => (
                  <span
                    key={lang}
                    className="text-[10px] bg-gray-100 px-2 py-0.5 rounded-full text-gray-600 font-medium"
                  >
                    {lang}
                  </span>
                ))}
            ...
            <div className="mt-auto p-4 bg-gray-50/50 flex gap-2">
              <button
                ...
                className="flex-1 py-2 text-sm font-bold text-gray-600 hover:bg-white hover:text-primary-600 hover:shadow-sm rounded-lg transition-all border border-transparent hover:border-gray-100"
              >
                Ver Perfil
              </button>
              <a
                href={buildWhatsAppLink(guide)}
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 py-2 flex items-center justify-center gap-2 text-sm font-bold text-white bg-green-500 hover:bg-green-600 rounded-lg shadow-green-500/20 shadow-md transition-all"
              >
