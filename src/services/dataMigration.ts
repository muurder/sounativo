...
export const migrateData = async () => {
  if (!supabase) {
    alert(
      'Cliente Supabase não está configurado. Verifique seu arquivo .env e as variáveis de ambiente.'
    );
    return;
  }
  ...
  try {
    ...
    for (const trip of MOCK_TRIPS) {
      ...
      if (!newAgencyId) {
        log(
          `⚠️ Viagem "${trip.title}" pulada: agência com ID mock "${trip.agencyId}" não encontrada.`
        );
        continue;
      }
    ...
    log('\n🏁 Migração finalizada.');
    alert(
      'Migração finalizada! Verifique o console (F12) para ver os logs detalhados do processo.'
    );
  }
};
