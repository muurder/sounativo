...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const _fetchGlobalAndClientProfiles = useCallback(async () => {
    ...
    if (!sb) {
      logger.warn(
        '[DataContext] Supabase not configured. Falling back to mock data for global profiles.'
      );
    ...
    try {
      ...
      const { data: agenciesData } = await sb
        .from('agencies')
        .select(
          'id, user_id, name, email, slug, whatsapp, cnpj, description, logo_url, is_active, is_guide, hero_mode, hero_banner_url, hero_title, hero_subtitle, custom_settings, subscription_status, subscription_plan, subscription_expires_at, website, phone, address, bank_info, cadastur, languages, specialties, certifications, experience_years, availability, deleted_at'
        )
      ...
      const { data: tripsData } = await sb.from('trips').select(`
      ...
      if (tripsData) {
        const mappedTrips: Trip[] = tripsData.map((t: any) => ({
          ...
          images: [], // Images loaded on-demand via fetchTripImages
          category: t.category,
          tags: t.tags || [],
          ...
          passengerConfig: t.passenger_config,
        }));
        setTrips(mappedTrips);
      }

      // 3. Fetch all Favorites (OPTIMIZED: only needed fields)
      const { data: favsData, error: favsError } = await sb
        .from('favorites')
      ...
      const { data: profilesData } = await sb
        .from('profiles')
        .select(
          'id, full_name, email, role, avatar_url, cpf, phone, birth_date, address, status, created_at, deleted_at'
        )
      ...
      if (profilesData) {
        const mappedClients: Client[] = profilesData.map((c: any) => ({
          ...
          favorites:
            allFavorites.filter((f: any) => f.user_id === c.id).map((f: any) => f.trip_id) || [],
          address: c.address || {},
      ...
      const { data: reviewsData } = await sb
        .from('agency_reviews')
        .select(
          'id, agency_id, client_id, booking_id, trip_id, rating, comment, tags, created_at, response, client:client_id(full_name, avatar_url)'
        ); // Join with profiles
      ...
      const { data: activityLogsData } = await sb
        .from('activity_logs')
      ...
      const { data: platformSettingsData, error: platformSettingsError } = await sb
      ...
      logger.log(
        '[DataContext] Global data loaded. Agencies:',
        agenciesData?.length,
        'Trips:',
        tripsData?.length,
        'Clients:',
      ...
      logger.warn(
        '[DataContext] Entrando em modo Offline/Fallback devido a erro de rede ou Supabase.'
      );
  ...
  const _fetchBookingsForCurrentUser = useCallback(async () => {
    ...
    if (!sb) {
      logger.warn(
        '[DataContext] Supabase not configured. Cannot fetch user-specific data, clearing bookings.'
      );
    ...
    try {
      ...
      if (user.role === UserRole.AGENCY) {
        ...
        logger.log(
          '[DataContext] _fetchBookingsForCurrentUser: Agency user, fetching bookings for trip IDs:',
          myTripIds
        );

        // FIX: Join with profiles (client:client_id) to get client name and avatar
        ({ data: bookingsData, error: bookingsError } = await sb
          .from('bookings')
          .select(
            '*, trips:trip_id(*, agencies:trips_agency_id_fkey(*)), client:client_id(full_name, avatar_url)'
          ) // Deep nesting + client join
          .in('trip_id', myTripIds));
      } else if (user.role === UserRole.CLIENT) {
        logger.log(
          '[DataContext] _fetchBookingsForCurrentUser: Client user, fetching bookings for client ID:',
          user.id
        );
        ...
        try {
          ...
          logger.warn(
            '[DataContext] booking_passengers fetch failed, fetching bookings without passengers:',
      ...
      if (bookingsData) {
        const augmentedBookings: Booking[] = bookingsData.map((b: any) => {
          ...
          return {
            ...
            _client: b.client
              ? { name: b.client.full_name, avatar: b.client.avatar_url }
              : undefined,
          } as any;
        });
  ...
  useEffect(() => {
    ...
    if (user?.id && !authLoading) {
      ...
      if (fetchedUserIdRef.current === user.id) {
        ...
        logger.log(
          '[DataContext] User changed (or first load), fetching user-specific data. User:',
          user.id
        );
    ...
    if (sb && user?.id) {
      ...
      const bookingChannel = sb
        .channel('bookings_changes_user_specific')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'bookings' }, (payload) => {
          logger.log(
            '[DataContext] Supabase Subscription: Booking change received for user-specific data!',
            payload
          );
          _fetchBookingsForCurrentUser();
        })
  ...
  const getTripBySlug = useCallback(
    (slugToFind: string) => {
  ...
  const getAgencyBySlug = useCallback(
    (slugToFind: string) => {
      if (!slugToFind || slugToFind.trim() === '') {
        return undefined;
      }
  ...
  const getAgencyPublicTrips = useCallback(
    (agencyId: string) => {
  ...
  const getReviewsByTripId = useCallback(
    (tripId: string) => {
      ...
      return agencyReviews
        ...
        .map((r) => ({
          ...
          clientName:
            r.clientName ||
            currentClients.find((c) => c.id === r.clientId)?.name ||
            'Cliente Desconhecido',
          clientAvatar:
            r.clientAvatar || currentClients.find((c) => c.id === r.clientId)?.avatar || undefined,
          agencyName:
            currentAgencies.find((a) => a.agencyId === r.agencyId)?.name || 'Agência Desconhecida',
  ...
  const getReviewsByAgencyId = useCallback(
    (agencyId: string) => {
      // Resolve clientName and agencyName dynamically when consuming reviews
      const currentClients = clientsRef.current;
      const currentAgencies = agenciesRef.current;
  ...
  const getReviewsByClientId = useCallback(
    (clientId: string) => {
      // Resolve clientName and agencyName dynamically when consuming reviews
      const currentClients = clientsRef.current;
      const currentAgencies = agenciesRef.current;
      ...
      return agencyReviews
        ...
        .map((r) => ({
          ...r,
          clientName:
            currentClients.find((c) => c.id === r.clientId)?.name || 'Cliente Desconhecido',
  ...
  const logActivity = useCallback(
    async (actionType: ActivityActionType, details: any) => {
      const sb = guardSupabase();
      if (sb && user?.id) {
  ...
  const addBooking = useCallback(
    async (booking: Booking): Promise<Booking | undefined> => {
      const sb = guardSupabase();
      if (!sb) {
      ...
      try {
        ...
        logActivity(ActivityActionType.BOOKING_CREATED, {
  ...
  ); // tripsRef, agenciesRef added to dependencies
  ...
  const addReview = useCallback(
    async (review: Review) => {
      ...
      try {
        ...
        logActivity(ActivityActionType.REVIEW_SUBMITTED, {
          reviewId: review.id,
  ...
  const addAgencyReview = useCallback(
    async (review: Partial<AgencyReview>) => {
      const sb = guardSupabase();
      if (!sb) {
      ...
      try {
        ...
        if (existingReview) {
          ...
          // Insert new review
          const { error: insertError } = await sb.from('agency_reviews').insert({
        ...
        await _fetchGlobalAndClientProfiles();
        logger.log(
          '[DataContext] Agency review added/updated successfully for agency:',
          review.agencyId
        );
      } catch (error: any) {
  ...
  const deleteAgencyReview = useCallback(
    async (id: string) => {
      const sb = guardSupabase();
      if (!sb) {
  ...
  const updateAgencyReview = useCallback(
    async (id: string, updates: Partial<AgencyReview>) => {
      const sb = guardSupabase();
      if (!sb) {
  ...
  const updateClientProfile = useCallback(
    async (userId: string, data: Partial<Client>) => {
      const sb = guardSupabase();
      if (!sb) {
  ...
  const updateAgencySubscription = useCallback(
    async (agencyId: string, status: string, plan: string, expiresAt?: string) => {
      const sb = guardSupabase();
      if (!sb) {
      ...
      logger.log(
        '[DataContext] Updating agency subscription ID:',
        agencyId,
        'Status:',
        status,
        'Plan:',
  ...
  const updateAgencyProfileByAdmin = useCallback(
    async (agencyId: string, data: Partial<Agency>) => {
      const sb = guardSupabase();
      if (!sb) {
        logger.warn(
          '[DataContext] Supabase not configured, cannot update agency profile by admin.'
        );
        return;
      }
    ...
    async (trip: Trip) => {
      ...
      try {
        ...
        const timeoutPromise = new Promise<never>((_, reject) =>
          setTimeout(
            () =>
              reject(new Error('O servidor demorou muito para criar o pacote. Tente novamente.')),
            15000
          )
        );
        ...
        if (trip.images && trip.images.length > 0) {
          ...
          setTrips((prevTrips) =>
            prevTrips.map((t) => (t.id === data.id ? { ...t, images: trip.images } : t))
          );
        }
      ...
      try {
        ...
        const timeoutPromise = new Promise<never>((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error('O servidor demorou muito para atualizar o pacote. Tente novamente.')
              ),
            15000
          )
        );
        ...
        if (trip.images && trip.images.length > 0) {
          setTrips((prevTrips) =>
            prevTrips.map((t) => (t.id === trip.id ? { ...t, images: trip.images } : t))
          );
        } else {
  ...
  const toggleTripStatus = useCallback(
    async (tripId: string) => {
      const sb = guardSupabase();
      if (!sb) {
  ...
  const toggleTripFeatureStatus = useCallback(
    async (tripId: string) => {
      const sb = guardSupabase();
      if (!sb) {
      ...
      try {
        ...
        logger.log(
          `[DataContext] Toggling featured status for trip ${tripId} to ${newFeaturedStatus}`
        );
        await sb.from('trips').update({ featured: newFeaturedStatus }).eq('id', trip.id);
        showToast(
          `Viagem ${newFeaturedStatus ? 'destacada' : 'removida do destaque'} com sucesso!`,
          'success'
        );
  ...
  const updateTripOperationalData = useCallback(
    async (tripId: string, data: OperationalData) => {
      const sb = guardSupabase();
      if (!sb) {
      ...
      setTrips((prevTrips) =>
        prevTrips.map((t) => (t.id === tripId ? { ...t, operationalData: data } : t))
      );
      ...
      try {
        ...
        try {
          ...
          const { data: tripData, error: rollbackError } = await sb
            .from('trips')
            .select('*')
            .eq('id', tripId)
            .single();
          ...
          if (!rollbackError && tripData) {
            ...
            setTrips((prevTrips) =>
              prevTrips.map((t) => (t.id === tripId ? (tripData as Trip) : t))
            );
          ...
          logger.error(
            '[DataContext] Critical: Rollback failed, forcing full refresh.',
            rollbackEx
          );
          _fetchGlobalAndClientProfiles();
        }
  ...
  const deleteUser = useCallback(
    async (id: string, role: string) => {
      ...
      try {
        ...
        if (authError) {
          ...
          // Just warn
          showToast(
            `Usuário DB excluído, mas conta de autenticação pode persistir: ${authError.message}`,
            'warning'
          );
        } else {
    ...
    [showToast, deleteTrip, logActivity, _fetchGlobalAndClientProfiles, guardSupabase, tripsRef]
  ); // tripsRef.current is used inside deleteUser, but not as dependency
  ...
  const deleteMultipleAgencies = useCallback(
    async (agencyPks: string[]) => {
      const sb = guardSupabase();
      if (!sb) {
      ...
      try {
        ...
        logActivity(ActivityActionType.DELETE_MULTIPLE_AGENCIES, {
          agencyPks,
    ...
    [showToast, deleteTrip, logActivity, _fetchGlobalAndClientProfiles, guardSupabase, tripsRef]
  ); // tripsRef.current is used here too
  ...
  const deleteAllData = useCallback(async () => {
    ...
    try {
      ...
      logger.log(
        `[DataContext] Found ${tripIds.length} trips, ${agencyIds.length} agencies, ${allUserIds.length} users to delete`
      );
  ...
  // Admin-specific functions
  const adminChangePlan = useCallback(
    async (agencyId: string, newPlan: 'BASIC' | 'PREMIUM') => {
      const sb = guardSupabase();
      if (!sb) {
  ...
  const adminBulkDeleteAgencies = useCallback(
    async (agencyIds: string[]) => {
  ...
  const adminBulkArchiveAgencies = useCallback(
    async (agencyIds: string[]) => {
      const sb = guardSupabase();
      if (!sb) {
  ...
  const adminBulkChangePlan = useCallback(
    async (agencyIds: string[], newPlan: 'BASIC' | 'PREMIUM') => {
      const sb = guardSupabase();
      if (!sb) {
      ...
      try {
        ...
        logActivity(ActivityActionType.AGENCY_SUBSCRIPTION_UPDATED, {
          agencyIds,
          newPlan,
          bulk: true,
        });
  ...
  const adminSuspendAgency = useCallback(
    async (agencyId: string) => {
      const sb = guardSupabase();
      if (!sb) {
  ...
  const getUsersStats = useCallback(
    async (userIds: string[]): Promise<UserStats[]> => {
      const sb = guardSupabase();
      if (!sb) {
    ...
    [bookings, agencyReviews, showToast, guardSupabase, clientsRef]
  ); // clientsRef added
  ...
  const updateMultipleAgenciesStatus = useCallback(
    async (ids: string[], status: string) => {
      const sb = guardSupabase();
      if (!sb) {
        logger.warn(
          '[DataContext] Supabase not configured, cannot update multiple agencies status.'
        );
        return;
      }
      ...
      try {
        ...
        logActivity(ActivityActionType.AGENCY_STATUS_TOGGLED, {
          agencyIds: ids,
  ...
  const logAuditAction = useCallback(
    async (action: string, details: string) => {
      ...
      if (!sb || !user?.email) {
        logger.warn(
          '[DataContext] Supabase not configured or user email missing, cannot log audit action.'
        );
        return;
      }
  ...
  const updateUserAvatarByAdmin = useCallback(
  ...
  const getAgencyStats = useCallback(
    async (agencyId: string): Promise<DashboardStats> => {
      const sb = guardSupabase();
      const zeros = {
        totalRevenue: 0,
        totalViews: 0,
      ...
      try {
        ...
        const relevantBookings = currentBookings.filter(
          (b) => b._trip?.agencyId === agencyId && b.status === 'CONFIRMED'
        );
  ...
  const getAgencyTheme = useCallback(
    async (agencyId: string): Promise<AgencyTheme | null> => {
      const sb = guardSupabase();
      if (!sb) {
      ...
      try {
        const { data, error } = await sb
          .from('agency_themes')
          .select(
            'agency_id, colors, font_pair, border_radius, button_style, header_style, background_image, background_blur, background_opacity, updated_at'
          )
  ...
  const saveAgencyTheme = useCallback(
    async (agencyId: string, theme: Partial<AgencyTheme>): Promise<boolean> => {
      const sb = guardSupabase();
      if (!sb) {
      ...
      try {
        ...
        ); // If a theme for this agency already exists, update it
  ...
  const incrementTripViews = useCallback(
    async (tripId: string) => {
      const sb = guardSupabase();
      if (!sb) {
      ...
      try {
        const { error } = await sb.rpc('increment_trip_views', {
  ...
  const updatePlatformSettings = useCallback(
    async (data: Partial<PlatformSettings>) => {
      const sb = guardSupabase();
      if (!sb) {
      ...
      try {
        ...
        if (data.platform_logo_url !== undefined)
          updates.platform_logo_url = data.platform_logo_url;
        ...
        if (data.background_transparency !== undefined)
          updates.background_transparency = data.background_transparency;
        if (data.default_settings !== undefined) updates.default_settings = data.default_settings;

        const { error } = await sb.from('platform_settings').update(updates).eq('id', 1);
  ...
  const fetchTripImages = useCallback(
    async (tripId: string, forceRefresh: boolean = false): Promise<string[]> => {
      const sb = guardSupabase();
      if (!sb) {
      ...
      logger.log(
        '[DataContext] Fetching images for trip ID:',
        tripId,
      ...
      try {
        ...
        setTrips((prevTrips) =>
          prevTrips.map((t) => (t.id === tripId ? { ...t, images: imageUrls } : t))
        );

        logger.log(
          '[DataContext] Trip images fetched successfully:',
          tripId,
          'Count:',
          imageUrls.length
        );
  ...
  const searchTrips = useCallback(
    async (params: SearchTripsParams): Promise<PaginatedResult<Trip>> => {
      ...
      if (params.sort) {
        switch (params.sort) {
          ...
          case 'DATE_ASC':
            ...
            break;
          case 'RATING':
            result.sort((a, b) => (b.tripRating || 0) - (a.tripRating || 0));
            break;
          case 'RELEVANCE':
          default:
            result.sort(
              (a, b) =>
                (b.views || 0) + (b.sales || 0) * 10 - ((a.views || 0) + (a.sales || 0) * 10)
            );
        }
      }
  ...
  const searchAgencies = useCallback(
    async (params: SearchAgenciesParams): Promise<PaginatedResult<Agency>> => {
      await new Promise((resolve) => setTimeout(resolve, 300));
      // FIX: Filter out inactive agencies and soft-deleted agencies
      let result = agenciesRef.current.filter(
        (a) => a.is_active === true && !a.deleted_at // Exclude soft-deleted agencies
      );

      if (params.query) {
        const q = normalizeText(params.query);
        result = result.filter(
          (a) => normalizeText(a.name).includes(q) || normalizeText(a.description).includes(q)
        );
      }
      ...
      if (params.specialty) {
        ...
        result = result.filter(
          (a) =>
            (a.customSettings?.tags &&
              a.customSettings.tags.some((t) => normalizeText(t).includes(q))) ||
      ...
      if (params.sort) {
        switch (params.sort) {
          ...
          default:
            result.sort(
              (a, b) =>
                (b.subscriptionPlan === 'PREMIUM' ? 1 : 0) -
                (a.subscriptionPlan === 'PREMIUM' ? 1 : 0)
            );
        }
      }
  ...
  const uploadPlatformLogo = useCallback(
    async (file: File): Promise<string | null> => {
      ...
      try {
        ...
        const {
          data: { publicUrl },
        } = sb.storage.from('agency-files').getPublicUrl(fileName);
  ...
  const sendBroadcast = useCallback(
    async (data: {
      ...
      try {
        ...
        logActivity(ActivityActionType.UPDATE_SETTINGS, {
  ...
  const getBroadcastsForAdmin = useCallback(async (): Promise<BroadcastWithInteractions[]> => {
    ...
    try {
      ...
      const broadcastsWithInteractions: BroadcastWithInteractions[] = broadcasts.map(
        (broadcast) => {
          const broadcastInteractions = (interactions || []).filter(
            (i) => i.broadcast_id === broadcast.id
          );
      ...
      logger.log(
        '[DataContext] Broadcasts fetched successfully:',
        broadcastsWithInteractions.length
      );
  ...
  const getUserNotifications = useCallback(
    async (userId: string, role: UserRole): Promise<BroadcastMessage[]> => {
      const sb = guardSupabase();
      if (!sb) {
  ...
  const interactWithBroadcast = useCallback(
    async (broadcastId: string, action: 'READ' | 'LIKE' | 'DELETE'): Promise<void> => {
      const sb = guardSupabase();
      if (!sb || !user) {
      ...
      try {
        ...
        if (existingInteraction) {
          ...
          const { error } = await sb.from('broadcast_interactions').insert(newInteraction);
  ...
  return (
    <DataContext.Provider
      value={{
        ...
        adminChangePlan,
        adminBulkDeleteAgencies,
        adminBulkArchiveAgencies,
        adminBulkChangePlan,
        adminSuspendAgency,
        getReviewsByTripId,
